...
#define INPUT_DOUBLE_TAP_DEFINE(inst)                                                              \
	...
	BUILD_ASSERT(!DT_INST_NODE_HAS_PROP(inst, single_tap_codes) ||                            \
		     DT_INST_PROP_LEN(inst, input_codes) ==                                        \
		     DT_INST_PROP_LEN(inst, single_tap_codes));                                    \
                                                                                                   \
	...
	IF_ENABLED(DT_INST_NODE_HAS_PROP(inst, single_tap_codes),                                  \
		   (static const uint16_t single_tap_codes_##inst[] =                              \
	...
	static const struct double_tap_config double_tap_config_##inst = {                         \
		...
		.single_tap_codes = COND_CODE_1(DT_INST_NODE_HAS_PROP(inst, single_tap_codes),    \
						(single_tap_codes_##inst), (NULL)),                \
