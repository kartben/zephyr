...
static int cmd_status(const struct shell *sh, size_t argc, char **argv)
{
	...
	shell_print(sh, "primary channel: %u \"%s\" hash=0x%02x",
		    ...
		    meshtastic_channels_primary_hash());
...
static int shell_parse_hex_psk(const struct shell *sh, const char *hex, uint8_t *out, size_t *out_len)
{
...
static int shell_apply_channel_psk(const struct shell *sh, meshtastic_Channel *ch,
...
static int cmd_channel_set(const struct shell *sh, size_t argc, char **argv)
{
	...
	if (argc < 3U) {
		shell_error(sh,
			    "usage: meshtastic channel set <index> "
...
SHELL_STATIC_SUBCMD_SET_CREATE(meshtastic_channel_cmds,
			       SHELL_CMD(list, NULL, SHELL_HELP("List channel slots.", NULL),
					 cmd_channel_list),
			       SHELL_CMD(show, NULL, SHELL_HELP("Show one channel slot.", "<index>"),
					 cmd_channel_show),
			       SHELL_CMD(set, NULL,
			       ...
			       SHELL_CMD(disable, NULL,
					 SHELL_HELP("Disable a channel slot.", "<index>"),
					 cmd_channel_disable),
...
SHELL_STATIC_SUBCMD_SET_CREATE(meshtastic_device_cmds,
			       SHELL_CMD(role, NULL,
					 SHELL_HELP("Get/set device role.", "[name]"),
					 cmd_device_role),
				     ...
				     const struct meshtastic_nodedb_node *node)
{
	...
	if (node->has_hops_away) {
		shell_print(sh,
			    "0x%08x last=%us snr=%d.%u hops=%u via=%s long=\"%s\" short=\"%s\"",
		...
		shell_print(sh,
			    "0x%08x last=%us snr=%d.%u hops=? via=%s long=\"%s\" short=\"%s\"",
			    node->num, node->last_heard_uptime_sec, scaled_whole(snr, 10),
			    scaled_fraction(snr, 10), node->via_mqtt ? "yes" : "no",
...
static void shell_print_optional_voltage(const struct shell *sh, const char *label,
					 bool has_value, float value)
{
	...
	shell_print(sh, "%s: %d.%03u", label, scaled_whole(mv, 1000),
		    scaled_fraction(mv, 1000));
}

static void shell_print_optional_tenths(const struct shell *sh, const char *label,
					bool has_value, float value)
{
	int32_t tenths;
	...
	shell_print(sh, "%s: %d.%u", label, scaled_whole(tenths, 10),
		    scaled_fraction(tenths, 10));
}
...
SHELL_STATIC_SUBCMD_SET_CREATE(meshtastic_nodedb_cmds,
			       SHELL_CMD(list, NULL, SHELL_HELP("List NodeDB entries.", NULL),
					 cmd_nodedb_list),
			       SHELL_CMD(show, NULL, SHELL_HELP("Show one NodeDB entry.",
								"<node|0xnode>"),
					 cmd_nodedb_show),
			       SHELL_SUBCMD_SET_END);
...
SHELL_STATIC_SUBCMD_SET_CREATE(meshtastic_gnss_cmds,
			       SHELL_CMD(send, NULL, SHELL_HELP("Send GNSS position.",
								"[dest|broadcast]"),
					 cmd_gnss_send),
			       SHELL_SUBCMD_SET_END);
...
SHELL_STATIC_SUBCMD_SET_CREATE(meshtastic_environment_cmds,
			       SHELL_CMD(send, NULL,
					 SHELL_HELP("Send environment telemetry.",
						    "[dest|broadcast]"),
