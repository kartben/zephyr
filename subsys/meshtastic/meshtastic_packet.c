				 ...
				 meshtastic_MeshPacket *mesh)
{
	...
	*mesh = (meshtastic_MeshPacket)meshtastic_MeshPacket_init_zero;
	...
	mesh->transport_mechanism = packet->via_mqtt ?
...
void meshtastic_mesh_packet_copy(meshtastic_MeshPacket *dst, const meshtastic_MeshPacket *src)
{
	...
	*dst = *src;
	 ...
	 */
	memcpy(&dst->decoded, &src->decoded,
	       MAX(sizeof(dst->decoded), sizeof(dst->encrypted)));
...
static int try_decrypt_wire_hash(uint8_t wire_hash, uint32_t from, uint32_t id,
				    ...
				    int8_t snr, struct meshtastic_packet *packet)
{
	...
	packet->hop_start = (hdr->flags & MESHTASTIC_FLAGS_HOP_START_MASK) >>
