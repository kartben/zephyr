...
static void tx_work_handler(struct k_work *work)
{
	...
	while (pop_fromradio(&frame)) {
		size_t wire_len = meshtastic_serial_encode_frame(frame.data, frame.len, wire,
								 sizeof(wire));
...
static int serial_rx_pop_frame(size_t *payload_len)
{
	...
	if (ring_buf_peek(&serial_rx_rb, hdr, MESHTASTIC_SERIAL_HEADER) != MESHTASTIC_SERIAL_HEADER) {
		return 0;
	}
