...
static void log_inject_mesh_packet(const char *phase, const meshtastic_MeshPacket *mesh)
{
	...
	LOG_DBG("inject %s: 0x%08x->0x%08x id=0x%08x port=%u %s enc=%zu hop=%u/%u ch=0x%02x via_mqtt=%d",
...
int meshtastic_inject_downlink_mesh_packet(const meshtastic_MeshPacket *mesh)
{
	...
	LOG_DBG("inject delivering locally port=%u payload_len=%zu",
		(unsigned int)packet.portnum, packet.payload_len);
