...
#define NODEINFO_BITFIELD_IS_KEY_MANUALLY_VERIFIED_MASK                                      \
	BIT(NODEINFO_BITFIELD_IS_KEY_MANUALLY_VERIFIED_SHIFT)
#define NODEINFO_BITFIELD_IS_MUTED_SHIFT 1
#define NODEINFO_BITFIELD_IS_MUTED_MASK BIT(NODEINFO_BITFIELD_IS_MUTED_SHIFT)
#define NODEINFO_BITFIELD_VIA_MQTT_SHIFT 2
#define NODEINFO_BITFIELD_VIA_MQTT_MASK BIT(NODEINFO_BITFIELD_VIA_MQTT_SHIFT)
#define NODEINFO_BITFIELD_IS_FAVORITE_SHIFT 3
#define NODEINFO_BITFIELD_IS_FAVORITE_MASK BIT(NODEINFO_BITFIELD_IS_FAVORITE_SHIFT)
#define NODEINFO_BITFIELD_IS_IGNORED_SHIFT 4
#define NODEINFO_BITFIELD_IS_IGNORED_MASK BIT(NODEINFO_BITFIELD_IS_IGNORED_SHIFT)
#define NODEINFO_BITFIELD_HAS_USER_SHIFT 5
#define NODEINFO_BITFIELD_HAS_USER_MASK BIT(NODEINFO_BITFIELD_HAS_USER_SHIFT)
#define NODEINFO_BITFIELD_IS_LICENSED_SHIFT 6
#define NODEINFO_BITFIELD_IS_LICENSED_MASK BIT(NODEINFO_BITFIELD_IS_LICENSED_SHIFT)
#define NODEINFO_BITFIELD_IS_UNMESSAGABLE_SHIFT 7
#define NODEINFO_BITFIELD_IS_UNMESSAGABLE_MASK BIT(NODEINFO_BITFIELD_IS_UNMESSAGABLE_SHIFT)
#define NODEINFO_BITFIELD_HAS_IS_UNMESSAGABLE_SHIFT 8
#define NODEINFO_BITFIELD_HAS_IS_UNMESSAGABLE_MASK BIT(NODEINFO_BITFIELD_HAS_IS_UNMESSAGABLE_SHIFT)
...
static void set_bit(meshtastic_NodeInfoLite *node, uint32_t mask, bool value)
{
	if (value) {
		node->bitfield |= mask;
	} else {
...
static bool has_bit(const meshtastic_NodeInfoLite *node, uint32_t mask)
{
	return (node->bitfield & mask) != 0U;
}
...
static void apply_user(struct nodedb_entry *entry, const meshtastic_User *user)
{
	...
	set_bit(node, NODEINFO_BITFIELD_HAS_USER_MASK, true);
	set_bit(node, NODEINFO_BITFIELD_IS_LICENSED_MASK, user->is_licensed);
	set_bit(node, NODEINFO_BITFIELD_HAS_IS_UNMESSAGABLE_MASK, user->has_is_unmessagable);
	set_bit(node, NODEINFO_BITFIELD_IS_UNMESSAGABLE_MASK,
			       ...
			       uint32_t now_sec)
{
	...
	set_bit(&entry->node, NODEINFO_BITFIELD_VIA_MQTT_MASK, packet->via_mqtt);
				     ...
				     const meshtastic_EnvironmentMetrics *src)
{
	...
	count = MIN((size_t)src->one_wire_temperature_count,
		    ARRAY_SIZE(dst->one_wire_temperature));
...
static void fill_snapshot(const struct nodedb_entry *entry, struct meshtastic_nodedb_node *out)
{
	...
	*out = (struct meshtastic_nodedb_node){0};
	...
	out->via_mqtt = has_bit(node, NODEINFO_BITFIELD_VIA_MQTT_MASK);
	out->has_hops_away = node->has_hops_away;
	out->hops_away = node->hops_away;

	out->has_user = has_bit(node, NODEINFO_BITFIELD_HAS_USER_MASK);
	...
	out->role = node->role;
	out->is_licensed = has_bit(node, NODEINFO_BITFIELD_IS_LICENSED_MASK);
	out->has_is_unmessagable = has_bit(node, NODEINFO_BITFIELD_HAS_IS_UNMESSAGABLE_MASK);
	out->is_unmessagable = has_bit(node, NODEINFO_BITFIELD_IS_UNMESSAGABLE_MASK);
