					  ...
					  const meshtastic_FromRadio *from)
{
	...
	LOG_DBG("%s FromRadio enqueue variant=%u len=%u pending=%u/%u", api->name,
		(unsigned int)from->which_payload_variant, frame.len, api->count,
		api->queue_size);
	k_mutex_unlock(&api->lock);
...
toradio_decoded:
	...
	LOG_DBG("%s ToRadio variant=%u len=%u", api->name,
		(unsigned int)to.which_payload_variant, (unsigned int)len);
...
void meshtastic_phoneapi_on_packet(const struct meshtastic_packet *packet)
{
	...
	LOG_DBG("FromRadio packet fan-out to %u transport(s), mesh id=%u", count,
		from.packet.id);
