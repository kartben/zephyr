...
/* Function checks if help is structured and formats it accordingly */
static void format_structured_help(const struct shell *sh, const char *item_help,
				   size_t terminal_offset)
{
	...
	if (structured->usage) {
		...
		z_shell_op_cursor_horiz_move(sh, terminal_offset + 7);
		formatted_text_print(sh, structured->usage, terminal_offset + 7, false);
		z_shell_fprintf(sh, SHELL_NORMAL, "\n");
	}
}
			    ...
			    uint16_t item_name_width, const char *item_help)
{
	...
	if (shell_help_is_structured(item_help)) {
		format_structured_help(sh, item_help, offset);
	} else {
			    ...
			    const struct shell_static_entry *cmd)
{
	...
	if (shell_help_is_structured(cmd->help)) {
		format_structured_help(sh, cmd->help, field_width);
	} else {
