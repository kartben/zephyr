...
foreach(root ${BOARD_ROOT})
  ...
  foreach(shield_yml ${shield_yml_files})
    ...
    # Use a unique YAML context name for each shield
    set(yaml_ctx_name "shield_data_${shield}")

    yaml_load(FILE ${shield_yml} NAME ${yaml_ctx_name})
    yaml_get(shield_data_content NAME ${yaml_ctx_name} KEY content)

    if(shield_data_content.shields)
      # Multiple shields format
      foreach(shield_info ${shield_data_content.shields})
        list(APPEND SHIELD_LIST ${shield_info.name})
        set(SHIELD_DIR_${shield_info.name} ${shield_path})
      endforeach()
    else()
  ...
  # Then look for Kconfig.shield files as fallback (legacy shields)
