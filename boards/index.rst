      ...
      document.addEventListener("DOMContentLoaded", function () {
        ...
        fetch(boardCatalogLink.href)
          ...
          .then((jsonData) => {
            ...
            // Populate capabilities checkboxes
            const capabilitiesContainer = document.getElementById("capabilities-container");

            // Sort capabilities with the most used first
            const capabilitiesArray = Array.from(capabilitiesSet);
            capabilitiesArray
              .sort((a, b) => {
                const countA = Object.values(data).filter((board) =>
                  board.supported.includes(a)
                ).length;
                const countB = Object.values(data).filter((board) =>
                  board.supported.includes(b)
                ).length;
