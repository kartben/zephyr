...
class Walker:
    ...
    def _is_within(self, srcAbspath, base_dir):
        """True if srcAbspath lives under base_dir (same common ancestor)."""
        if not base_dir:
            return False
        try:
            return os.path.commonpath([srcAbspath, base_dir]) == base_dir
        except ValueError:
