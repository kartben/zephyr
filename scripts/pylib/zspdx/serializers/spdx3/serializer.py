...
class SPDX3Serializer:
    ...
    def _other_identifier(self, value: str) -> spdx.ExternalIdentifier:
        ...
        ``value`` uses a Zephyr-defined "<scheme>:<value>" form, so the Zephyr
        Project is recorded as the scheme's issuing authority.
        """
        ext_id = spdx.ExternalIdentifier()
        ext_id.externalIdentifierType = spdx.ExternalIdentifierType.other
        ext_id.identifier = value
        ext_id.issuingAuthority = self._ZEPHYR_ID_AUTHORITY
        return ext_id
    ...
    def _create_twister_verifications(self):
        ...
        for record in records:
            ...
            for block in record.test_items:
                self._emit_test_verification(block, package, catalog, record=record, req_doc=True)
    ...
    def _get_or_create_test_package(self, record):
        ...
        if record.run_id:
            package.externalIdentifier.append(
                self._other_identifier(f"twister-run:{record.run_id}")
            )
        self.elements.append(package)
