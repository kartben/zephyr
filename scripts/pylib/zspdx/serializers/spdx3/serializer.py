...
class SPDX3Serializer:
    ...
    def __init__(self, sbom_graph: SBOMGraph, spdx_version=None):
        ...
        self.build_info = sbom_graph.metadata.get("build_info", {}) if sbom_graph.metadata else {}
    ...
    def _create_build_tools(self):
        ...
        Build tools are ``Tool`` elements, not ``Agent``s (an Agent creates SPDX data). Idempotent;
        ...
        # (tool key, display label, build_info path key, build_info version key)
        others = [
        ...
        for key, label, path_key, version_key in others:
    ...
    def _create_build_object(self):
        ...
        Idempotent; a no-op when no build information exists. Start/end times are left unset so
        builds stay reproducible (the spec allows omitting them).
        """
    ...
    def _create_build_output_relationships(self):
        ...
        for component in self.sbom_data.components.values():
            ...
            if component.metadata.get("target_type") == "EXECUTABLE":
                # final images are outputs of the overall build
    ...
    def _create_target_build(self, component: SBOMComponent, build_file) -> None:
        ...
        The sub-build ``hasOutput`` the target's artifact, ``hasInput`` its source files and
        ``usesTool`` the specific compiler(s) and archiver/linker that produced it. Sub-builds
        connect to the overall build through the artifacts they share, which already carry
        ``generates``/``hasStaticLink`` edges.
        """
        namespace = self.sbom_data.namespace_prefix.rstrip("/")
        ...
        # the target's sources are the artifact's GENERATED_FROM endpoints
        source_ids = []
        ...
        if source_ids:
            self._new_build_relationship(
                spdx.RelationshipType.hasInput, target_build._id, source_ids
            )
    ...
    def _create_relationships(self):
        ...
        Every relationship is attached to its ``from`` element, so iterating the
        components and files covers the whole graph; iterating
        ``sbom_data.relationships`` as well would duplicate each one.
        """
        for component in self.sbom_data.components.values():
