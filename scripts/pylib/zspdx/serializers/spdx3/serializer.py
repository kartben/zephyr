...
class SPDX3Serializer:
    ...
    def _encode_elements(self, elements: list) -> list:
        """Encode each element to its JSON-LD dict, skipping any that fail."""
        elements_data = []
    ...
    def _simplify_document_references(self, elements_data: list):
        """Replace inlined element/rootElement objects with bare ID references."""
        for elem in elements_data:
            if elem.get("type") != "SpdxDocument":
                continue
            for key in ("element", "rootElement"):
                if key in elem:
                    elem[key] = self._reference_ids(elem[key])

    @staticmethod
    def _reference_ids(refs: list) -> list:
        """Reduce a list of element references (objects or strings) to ID strings."""
        ids = []
        for ref in refs:
            if isinstance(ref, dict):
                ref_id = ref.get("spdxId") or ref.get("@id")
                if ref_id:
                    ids.append(ref_id)
