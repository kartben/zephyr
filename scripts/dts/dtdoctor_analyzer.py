...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="number of related symbols collapsed into this diagnostic",
    )
...
def parse_dt_macro(symbol: str) -> dict:
    ...
    if symbol.startswith('N_NODELABEL_'):
        ...
        result['type'] = 'path'
        # For paths, we want to keep the N... prefix
        rest = symbol
    else:
        # Unknown format
        return None
    ...
    if '_P_' in rest:
        ...
        #   <prop>_NAME_<name>_VAL_<cell>  (named phandle-array entry, cell access)
        #   <prop>_NAME_<name>_PH_ORD      (named phandle-array entry, phandle ordinal)
        #   <prop>_NAME_<name>             (named phandle-array entry)
        #   <prop>_IDX_<n>_VAL_<cell>      (indexed phandle-array entry, cell access)
        #   <prop>_IDX_<n>                 (indexed phandle-array entry)
        #   <prop>                          (direct property access)

        # Check for _NAME_ pattern (named entry in phandle-array, e.g. dmas by name)
        name_val_match = re.match(r'(.+?)_NAME_(.+?)_VAL_(\w+)$', prop_part)
        name_ph_match = (
            re.match(r'(.+?)_NAME_(.+?)_PH_ORD$', prop_part)
            if not name_val_match
            else None
        )
        name_only_match = (
            re.match(r'(.+?)_NAME_(\w+)$', prop_part)
            if not name_val_match and not name_ph_match
            else None
        )

        if name_val_match:
            result['property'] = name_val_match.group(1)
            result['phandle_name'] = name_val_match.group(2)
            result['cell'] = name_val_match.group(3)
        elif name_ph_match:
            result['property'] = name_ph_match.group(1)
            result['phandle_name'] = name_ph_match.group(2)
            result['phandle_ord'] = True
        elif name_only_match:
            result['property'] = name_only_match.group(1)
            result['phandle_name'] = name_only_match.group(2)
        else:
            ...
            if idx_match:
                ...
                # No sub-accessor, just property name
        ...
        # Format: NAME_<name>_VAL_<cell> or IDX_<idx>_VAL_<cell>
        ...
        # No property access or irq, just node identifier
    ...
    if node_type == 'nodelabel':
        ...
        for node in edt.nodes:
            for label in node.labels:
                # Convert label using gen_defines logic
                label_ident = gen_defines.str2ident(label)
        ...
        # Search for node matching the z_path_id from gen_defines
        target_ident = node_ident
        for node in edt.nodes:
            if gen_defines.node_z_path_id(node) == target_ident:
                return node
...
def _diagnose_missing_property(node: edtlib.Node, prop_name: str) -> list[str]:
    ...
    # Use the binding schema (prop2specs) to show all properties the node
    # *could* have, not just the ones currently set.  This is more useful
    # because it tells the user whether the property is even valid for this
    # node and what alternatives exist.
    binding = getattr(node, '_binding', None)
    if binding and hasattr(binding, 'prop2specs') and binding.prop2specs:
        schema_props = binding.prop2specs
        if prop_name in schema_props:
            spec = schema_props[prop_name]
            lines.append(
                f"Property '{prop_name}' is defined in the binding, "
                f"but it is not set on the node."
            )
            lines.append(f"\nAdd '{prop_name}' to the node in your devicetree.")
        else:
            lines.append(
                f"The binding for '{node.compats[0] if node.compats else '?'}' "
                f"does not define a '{prop_name}' property."
            )
            lines.append("\nProperties defined by the binding:")
            for pname in sorted(schema_props.keys()):
                marker = " (set)" if pname in node.props else ""
                lines.append(f" - {pname}{marker}")
    elif node.props:
        lines.append("Properties currently set on this node:")
        for pname in sorted(node.props.keys()):
            lines.append(f" - {pname}")
        lines.append("\nCheck your devicetree binding and node definition.")
    else:
        lines.append("This node has no properties set and no binding was found.")

    return lines


def _diagnose_named_access(
    node: edtlib.Node, prop: edtlib.Property, parsed: dict
) -> list[str]:
    """Diagnose errors accessing a phandle-array property by name."""
    phandle_name = ident2str(parsed['phandle_name'])

    # The names property follows the DT convention: singular form + '-names'
    # e.g., dmas -> dma-names, clocks -> clock-names
    names_prop_name = prop.name.rstrip('s') + '-names'

    if names_prop_name not in node.props:
        return [
            f"Named entry '{phandle_name}' requested from property '{prop.name}' "
            f"on node '{format_node(node)}',\n"
            f"but there is no '{names_prop_name}' property to resolve names.\n",
            f"Add a '{names_prop_name}' property to the node in your devicetree.",
        ]

    names = node.props[names_prop_name].val
    if isinstance(names, list) and phandle_name not in names:
        lines = [
            f"Named entry '{phandle_name}' not found in '{names_prop_name}' "
            f"on node '{format_node(node)}'.\n"
        ]
        lines.append(f"Available names in '{names_prop_name}':")
        for n in names:
            lines.append(f" - {n}")
        return lines

    # Name was found; check cell access if applicable
    if isinstance(names, list) and 'cell' in parsed:
        idx = names.index(phandle_name)
...
def _diagnose_property_access(node: edtlib.Node, parsed: dict) -> list[str]:
    ...
    if 'phandle_name' in parsed:
        return _diagnose_named_access(node, prop, parsed)
...
def _diagnose_irq_access(node: edtlib.Node, parsed: dict) -> list[str]:
    ...
    if 'cell' in parsed:
        ...
        lines = [
            f"Cell '{cell_name}' not found in interrupt specifier for node '{format_node(node)}'.\n"
        ]
        if entry.data:
             lines.append("Available cells:")
             for cname in entry.data.keys():
...
def _print_diagnostic(lines: list[str], related_count: int = 0) -> None:
    """Print a DT Doctor diagnostic box, optionally noting collapsed related errors."""
    if related_count > 1:
        lines.append(
            f"\n({related_count} related build errors collapsed into this diagnostic.)"
        )
...
def main() -> int:
    ...
    if m:
        ...
        try:
            ...
            _print_diagnostic(lines, args.related_count)
            return 0
