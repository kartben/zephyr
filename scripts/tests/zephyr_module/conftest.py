...
class ManifestLab:
    ...
    def add_repo(self, name: str, files: dict[str, str] | None = None) -> str:
        ...
        git(["add", "-A"], work)
        git(
            ["-c", "user.email=ci@zephyrproject.org", "-c", "user.name=CI", "commit", "-q",
             "-m", "initial commit"],
            work,
        )
    ...
    def fork_repo(self, name: str, source: str) -> None:
        ...
        git(["clone", "-q", "--bare", str(self.url(source)), str(self.remotes / f"{name}.git")],
            self.root)
...
def west(args: list[str], topdir: Path, check: bool = True) -> subprocess.CompletedProcess:
    ...
    return subprocess.run(
        ["west", *args], cwd=topdir, check=check, capture_output=True, text=True
    )
