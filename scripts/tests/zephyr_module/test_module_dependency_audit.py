...
def test_a_module_path_used_without_a_declaration_is_undeclared(modules):
    ...
    assert ("cmake", "drivers/sensor/vendor") in {
        (ref.kind, ref.consumer) for ref in vendor.undeclared
    }
...
def test_depending_on_a_module_conditional_symbol_declares_the_dependency(modules):
    ...
    assert tdk.conditional_symbols == [
        "TDK_FIFO_SUPPORT",
        "TDK_LOW_LEVEL_DRIVER",
...
def test_an_ungated_glue_symbol_declares_nothing(modules):
    ...
    assert ("capability", "soc/vendor/Kconfig") in {
...
def test_a_tab_after_if_still_gates(tmp_path):
    ...
    modules = audit_tool.audit(tmp_path / "zephyr", tmp_path / "zephyr" / "west.yml", sources=False)
...
def test_glue_split_across_files_belongs_to_the_module_it_gates_on(tmp_path):
    """A module declares its presence symbol once, however many glue files it has."""
    write_tree(
        tmp_path,
        {
            "zephyr/west.yml": "manifest:\n  projects:\n    - name: hal_split\n"
            "      path: modules/hal/split\n",
            "zephyr/modules/Kconfig.split_a": (
                "config ZEPHYR_HAL_SPLIT_MODULE\n\tbool\n\n"
                "config HAS_SPLIT_A\n\tbool\n\tdepends on ZEPHYR_HAL_SPLIT_MODULE\n"
            ),
            "zephyr/modules/Kconfig.split_b": (
                "config HAS_SPLIT_B\n\tbool\n\tdepends on ZEPHYR_HAL_SPLIT_MODULE\n"
            ),
            "zephyr/drivers/split/Kconfig": (
                "config SPLIT_DRIVER\n\tbool \"Split driver\"\n\tselect HAS_SPLIT_B\n"
            ),
        },
    )
    workspace = tmp_path / "zephyr"

    modules = audit_tool.audit(workspace, workspace / "west.yml", sources=False)

    assert "HAS_SPLIT_B" in modules["hal_split"].conditional_symbols
    assert ("select", "drivers/split/Kconfig") in {
        (ref.kind, ref.path) for ref in modules["hal_split"].declared
    }
    assert audit_tool.unattributed_glue(audit_tool.read_tree(workspace), modules) == {}


def test_a_second_declaration_must_repeat_the_dependency(tmp_path):
    """Kconfig ORs a symbol's dependencies across every place it is declared.

    A SoC or board tree that redeclares a glue symbol to set a default for
    itself therefore cancels the gate, unless it repeats the dependency.
    """
    glue = (
        "config ZEPHYR_HAL_TWICE_MODULE\n\tbool\n\n"
        "config TWICE_HAL\n\tbool\n\tdepends on ZEPHYR_HAL_TWICE_MODULE\n"
    )

    def audit(name, override):
        write_tree(
            tmp_path / name,
            {
                "zephyr/west.yml": "manifest:\n  projects:\n    - name: hal_twice\n"
                "      path: modules/hal/twice\n",
                "zephyr/modules/hal_twice/Kconfig": glue,
                "zephyr/soc/vendor/Kconfig.defconfig": override,
                "zephyr/soc/vendor/Kconfig": "config SOC_TWICE\n\tbool\n\tselect TWICE_HAL\n",
            },
        )
        base = tmp_path / name / "zephyr"
        return audit_tool.audit(base, base / "west.yml", sources=False)["hal_twice"]

    cancelled = audit("cancelled", "config TWICE_HAL\n\tdefault y\n")
    repeated = audit(
        "repeated",
        "config TWICE_HAL\n\tdepends on ZEPHYR_HAL_TWICE_MODULE\n\tdefault y\n",
    )

    assert "TWICE_HAL" not in cancelled.conditional_symbols
    assert "TWICE_HAL" in repeated.conditional_symbols
    assert ("select", "soc/vendor/Kconfig") in {(ref.kind, ref.path) for ref in repeated.declared}
    assert "soc/vendor" not in repeated.undeclared_consumers
...
def test_a_module_own_glue_is_not_a_consumer(modules):
    ...
    assert all(not ref.path.startswith("modules/hal_tdk/") for ref in modules["hal_tdk"].references)
