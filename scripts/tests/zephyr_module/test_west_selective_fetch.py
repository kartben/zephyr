...
def upstream(lab: ManifestLab) -> ManifestLab:
    ...
    lab.add_repo("zephyr", {"west.yml": f"""\
        manifest:
    ...
    lab.add_repo("downstream", {"west.yml": f"""\
        manifest:
          projects:
            - name: zephyr
              url: {lab.url('zephyr')}
...
def test_downstream_overrides_survive_a_selective_fetch(upstream: ManifestLab,
...
def test_groups_cannot_be_combined_with_an_import(lab: ManifestLab, tmp_path_factory):
    ...
    isolated.add_repo("manifest-repo", {"west.yml": f"""\
        manifest:
          group-filter: [-optional]
          projects:
            - name: imported
