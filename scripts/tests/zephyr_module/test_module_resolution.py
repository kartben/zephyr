...
def test_strict_activation_leaves_unused_modules_out():
    ...
    resolution = zephyr_module.resolve_modules(
        WORKSPACE_MODULES, required={"hal_tdk": ["ICM42X70"]},
...
def test_strict_activation_follows_module_dependencies():
    ...
    resolution = zephyr_module.resolve_modules(
        WORKSPACE_MODULES, required={"hal_vendor": ["SOC_VENDOR"]},
        activation=zephyr_module.ACTIVATION_STRICT)
...
def test_modules_the_user_supplies_are_deliberate():
    ...
    resolution = zephyr_module.resolve_modules(
        WORKSPACE_MODULES, explicit=["modules/hal_tdk"],
        activation=zephyr_module.ACTIVATION_STRICT)
...
def test_a_required_module_that_is_not_there_is_reported():
    ...
    resolution = zephyr_module.resolve_modules(
        WORKSPACE_MODULES, required={"hal_absent": ["ICM42X70"]},
        activation=zephyr_module.ACTIVATION_STRICT)
...
def test_the_resolution_is_machine_readable():
    resolution = zephyr_module.resolve_modules(
        WORKSPACE_MODULES, required={"hal_vendor": ["SOC_VENDOR"], "gone": ["SOC_VENDOR"]},
        activation=zephyr_module.ACTIVATION_STRICT)
    ...
    assert modules["hal_vendor"] == {
        "name": "hal_vendor", "path": "modules/hal_vendor", "available": True,
        "active": True, "reason": "required", "required": True,
...
MODULE_YAML = {
    ...
    "cmsis": "name: cmsis\nbuild:\n  cmake: zephyr\n  kconfig: zephyr/Kconfig\n",
    "hal_vendor": ("name: hal_vendor\nbuild:\n  cmake: zephyr\n  kconfig: zephyr/Kconfig\n"
                   "  depends:\n    - cmsis\n"),
    "board_provider": ("name: board_provider\nbuild:\n  cmake: zephyr\n"
                       "  settings:\n    board_root: .\n"),
}
...
def run_zephyr_module(workspace, tmp_path, *args):
    ...
    outputs = {name: tmp_path / name for name in
               ("Kconfig.modules", "zephyr_modules.txt", "zephyr_settings.txt", "modules.json")}
    subprocess.run(
        [sys.executable, str(zephyr_module.__file__),
         "--zephyr-base", str(workspace / "zephyr"),
         "--kconfig-out", str(outputs["Kconfig.modules"]),
         "--cmake-out", str(outputs["zephyr_modules.txt"]),
         "--settings-out", str(outputs["zephyr_settings.txt"]),
         "--modules-out", str(outputs["modules.json"]),
         *args],
...
def test_a_populated_workspace_activates_everything_by_default(workspace, tmp_path):
    ...
    for name in MODULE_YAML:
        ...
        assert f"config ZEPHYR_{name.upper()}_MODULE\n\tbool\n\tdefault y" \
...
def test_a_module_supplied_by_hand_satisfies_the_requirement(workspace, tmp_path):
    ...
    generated = run_zephyr_module(
        ...
        "--extra-modules", str(workspace / "modules" / "hal_nxp"))
    report = json.loads(generated["modules.json"])
    modules = {entry["name"]: entry for entry in report["modules"]}
...
def test_a_missing_module_fails_the_build_early(workspace, tmp_path):
    ...
    with pytest.raises(subprocess.CalledProcessError) as failure:
        run_zephyr_module(workspace, tmp_path, "--activation", "strict",
                          "--required", "hal_absent")
...
def test_module_dependencies_survive_parsing(workspace):
    ...
    modules = zephyr_module.parse_modules(str(workspace / "zephyr"),
                                          modules=[str(workspace / "modules" / name)
                                                   for name in MODULE_YAML])
...
def test_requirements_flow_from_a_file_into_the_resolution(workspace, tmp_path):
    ...
    write_tree(tmp_path, {"requirements.json": json.dumps({
        "schema_version": 1,
        "required": [{"name": "hal_vendor", "required_by": ["SOC_SERIES_VENDOR"]}],
    })})

    generated = run_zephyr_module(workspace, tmp_path, "--activation", "strict",
                                  "--required-file", str(tmp_path / "requirements.json"))
    report = json.loads(generated["modules.json"])
    modules = {entry["name"]: entry for entry in report["modules"]}
