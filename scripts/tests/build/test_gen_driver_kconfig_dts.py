...
EXTRACTABLE = [
    ...
    '\n\n# comment\ncompatible: vnd,foo\n',
    'compatible: "vnd,devé"\n',
    # cases resolved through the prefix parse: quotes, flow collections or
    # other 'compatible' spellings on earlier lines
    "description: it's quoted\ncompatible: vnd,foo\n",
    'include: [base.yaml]\ncompatible: vnd,foo\n',
    'x: [a,\n b]\ncompatible: vnd,foo\n',  # multi-line flow, closed in time
    'compatible : vnd,first\ncompatible: vnd,second\n',  # first spelling wins
    'some-prop: "compatible"\ncompatible: "vendor,chip"\n',  # word as a value
]
...
ABSTAIN = [
    ...
    # column 0 'compatible:' lines that are data inside a construct opened on
    # an earlier line
    'x: [\ncompatible: vnd,data\n]\n',
    'x: {\ncompatible: vnd,data\n}\n',
    'x: "spans\ncompatible: vnd,data\nlines"\n',
    "x: 'spans\ncompatible: vnd,data\nlines'\n",
    'x: [\ncompatible: decoy]\ncompatible: vnd,real\n',
    # null-valued 'compatible' whose value continues as a zero-indented block
    # sequence past the matched line
    "description: it's quoted\ncompatible:\n- top: item\n",
]
...
# Document fragments combined pairwise by test_fragment_combinations();
# derived from fuzzing that surfaced corner cases the curated lists missed
FRAGMENTS = [
    'compatible: vnd,foo\n',
    'compatible: "vnd,bar"\n',
    'compatible: |\n  vnd,baz\n',
...
@pytest.mark.parametrize("first", FRAGMENTS)
def test_fragment_combinations(first):
    # Right-or-abstain over every pairwise combination of the fragments. When
    # the combined document is invalid YAML the fast path is allowed to
    # extract from the valid part (see the comment in main()); the invariant
    # only applies to valid documents.
    for second in FRAGMENTS:
        ...
        if fast is None:
            continue
        try:
            yaml.compose(contents, Loader=SafeLoader)
        except yaml.YAMLError:
            continue
        assert fast == compose_compat(contents), repr(contents)
