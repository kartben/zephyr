...
from zspdx.traceability import load_traceability, parse_traceability  # noqa: E402
...
def _sample_needs():
    """A sphinx-needs export with one requirement of each kind of linked need."""
    return [
        {"id": "ZEP-SYRS-1", "type": "req", "title": "Threading"},
        {
            "id": "ZEP-SRS-1-1",
            "type": "req",
            ...
            "rtype": "Functional",
            "trace": ["ZEP-SYRS-1"],
            "fulfills_back": ["DESIGN-THREADS"],
            "implements_back": ["k_thread_create"],
            "validates_back": ["threads__test_thread_start"],
        },
        {
            "id": "DESIGN-THREADS",
            "type": "design_need",
            "title": "Thread design",
            "fulfills": ["ZEP-SRS-1-1"],
        },
        {
            "id": "k_thread_create",
            "type": "impl",
            "title": "Create a thread.",
            "implements": ["ZEP-SRS-1-1"],
        },
        {
            "id": "threads__test_thread_start",
            "type": "test",
            "title": "start",
            "content": "Documented at :c:func:`threads__test_thread_start`.\n\n"
...
def test_traceability_parse_classifies_needs():
    graph = parse_traceability(_sample_needs())
    assert set(graph.requirements) == {"ZEP-SYRS-1", "ZEP-SRS-1-1"}
    assert set(graph.designs) == {"DESIGN-THREADS"}
    # the implementation symbol is a need of its own, not a test
    ...
    assert srs.component == "Threads" and srs.fulfilled_by == ["DESIGN-THREADS"]
    assert not srs.is_system and graph.requirements["ZEP-SYRS-1"].is_system
    assert graph.implementation_symbols() == {"k_thread_create"}
    # the test's prose body is kept, without the Doxygen back-reference line
    assert graph.tests["threads__test_thread_start"].details == (
        "Spawns a thread and checks it runs."
    )


def test_traceability_load_unwraps_the_version_envelope(tmp_path):
    export = tmp_path / "needs.json"
    export.write_text(
        json.dumps(
            {
                "current_version": "1.0",
                "versions": {"1.0": {"needs": {n["id"]: n for n in _sample_needs()}}},
            }
        )
    )
    graph = load_traceability(str(export))
    assert set(graph.designs) == {"DESIGN-THREADS"}
    assert graph.requirements["ZEP-SRS-1-1"].implemented_by == ["k_thread_create"]
...
def _fs_metadata(tmp_path, *, covered=True):
    ...
    traceability = parse_traceability(
        [
            {"id": "ZEP-SYRS-1", "type": "req", "title": "Sys"},
            {
                "id": "ZEP-SRS-1-1",
                "type": "req",
                "title": "Foo",
                ...
                "rtype": "Functional",
                "trace": ["ZEP-SYRS-1"],
                "validates_back": ["suite__test_foo"],
            },
            {
                "id": "DESIGN-X",
                "type": "design_need",
            ...
            {"id": "k_foo", "type": "impl", "title": "foo", "implements": ["ZEP-SRS-1-1"]},
            {
                "id": "suite__test_foo",
                "type": "test",
                "title": "exercises foo",
