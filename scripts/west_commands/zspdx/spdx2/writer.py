...
"""SPDX 2.x tag-value format writer."""

from datetime import datetime

from west import log

from zspdx.util import (
    normalize_spdx_name,
...
def write_relationship(f, rln):
    """
    Output tag-value SPDX 2.x content for the given Relationship object.

    Arguments:
        f: file handle for SPDX document
        rln: Relationship object being described
    """
    ...
    f.write(f"Relationship: {refA} {rln.rlnType} {refB}\n")


def write_file(f, bf):
    """
    Output tag-value SPDX 2.x content for the given File object.

    Arguments:
        f: file handle for SPDX document
        bf: File object being described
    """
    spdx_id = normalize_spdx_name(bf.spdxID)
...
""")
    ...
    if len(bf.rlns) > 0:
        for rln in bf.rlns:
            write_relationship(f, rln)
        f.write("\n")


def write_package(f, pkg, spdx_version=SPDX_VERSION_2_3):
    """
    Output tag-value SPDX 2.x content for the given Package object.

    Arguments:
        f: file handle for SPDX document
        pkg: Package object being described
        spdx_version: SPDX specification version
    """
    # Update package metadata based on provided CPE reference
    for ref in pkg.cfg.externalReferences:
        if is_cpe23_reference(ref):
    ...
    spdx_normalized_name = normalize_spdx_name(pkg.cfg.name)
    spdx_id = normalize_spdx_name(pkg.cfg.spdxID)
...
""")
    ...
    if len(pkg.cfg.url) > 0:
        downloadUrl = generate_download_url(pkg.cfg.url, pkg.cfg.revision)
...
def write_other_license(f, lic):
    """
    Output tag-value SPDX 2.x content for a custom license.

    Arguments:
        f: file handle for SPDX document
        lic: custom license ID being described
    """
...
def write_document(f, doc, spdx_version=SPDX_VERSION_2_3):
    """
    Output tag-value SPDX 2.x content for the given Document object.

    Arguments:
        f: file handle for SPDX document
        doc: Document object being described
        spdx_version: SPDX specification version
    """
    spdx_normalized_name = normalize_spdx_name(doc.cfg.name)
...
""")
    ...
    for pkg in doc.pkgs.values():
        write_package(f, pkg, spdx_version)

    # write other license info, if any
    if len(doc.customLicenseIDs) > 0:
        for lic in sorted(list(doc.customLicenseIDs)):
            write_other_license(f, lic)


def write_spdx(spdx_path, doc, spdx_version=SPDX_VERSION_2_3):
    """
    Open SPDX 2.x document file for writing, write the document, and calculate
    ...
    try:
        log.inf(f"Writing SPDX {spdx_version} document {doc.cfg.name} to {spdx_path}")
        with open(spdx_path, "w") as f:
            write_document(f, doc, spdx_version)
    except OSError as e:
