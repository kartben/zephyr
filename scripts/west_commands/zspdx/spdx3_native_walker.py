...
class SPDX3NativeWalker:
    """
    Native SPDX3 walker that directly creates SPDX3 objects without pending references.

    This approach is much cleaner than the two-phase pending approach because:
    1. We create SPDX3 objects directly as we discover them
    2. We use object references instead of string IDs
    3. We build relationships directly between objects
    4. No need for pending queues or complex resolution logic
    """
    ...
    def _process_target(self, target):
        ...
        for source_group in target.sourceGroups:
            for source in source_group.sources:
                source_file = self._process_source_file(source, package)
    ...
    def _process_source_file(self, source, package):
