...
class SPDX3Generator:
    ...
    def generate_spdx3(self) -> bool:
        ...
        try:
            ...
            # Set up scanner configuration and scan documents to populate copyright information
            from zspdx.scanner import ScannerConfig, scanDocument

            scanner_cfg = ScannerConfig()

            # Scan each document from walker to populate copyright information
            if self.config.includeSDK and hasattr(walker, 'docSDK'):
                scanDocument(scanner_cfg, walker.docSDK)
            scanDocument(scanner_cfg, walker.docApp)
            scanDocument(scanner_cfg, walker.docZephyr)
            scanDocument(scanner_cfg, walker.docBuild)
            if hasattr(walker, 'docModulesExtRefs') and walker.docModulesExtRefs:
                scanDocument(scanner_cfg, walker.docModulesExtRefs)
