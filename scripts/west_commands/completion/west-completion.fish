...
function __zephyr_west_complete_help
    ...
    set -l ext_cmds "completion" "display shell completion scripts" \
                    ...
                    "modules" "list or fetch modules required by a build" \
...
# modules
complete -c west -n "__zephyr_west_use_subcommand; and __zephyr_west_check_if_in_workspace" -ra modules -d "list or fetch modules required by a build"
...
complete -c west -n "__zephyr_west_seen_subcommand_from modules" -l app -r -d "application directory"
