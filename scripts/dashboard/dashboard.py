...
class ZephyrDashboard:
    ...
    def __init__(
        ...
        self._init_kconfigs()
        self._init_sysinit()
        self._init_hardening(hardening_report)
    ...
    def _init_hardening(self, hardening_report):
        '''
        Load the hardening report JSON produced by
        scripts/kconfig/hardenconfig.py (HARDENCONFIG_JSON). The hardening
        page is skipped when no report is available, so standalone
        invocations without --hardening-report keep working.
        '''
        self.hardening = None
        self.hardening_fail_count = 0
        if not hardening_report:
            return

        try:
            with open(hardening_report, encoding='utf-8') as fd:
                self.hardening = json.load(fd)
        except (OSError, json.JSONDecodeError) as e:
            logger.warning(f'Unable to load hardening report at {hardening_report}: {e}')
            return

        self.hardening_fail_count = self.hardening['fail_count']

        # Add a link to the definition of CWE and CVE references.
        for option in self.hardening['options']:
            for ref in option['references']:
                match = re.fullmatch(r'CWE-(\d+)', ref['id'], re.IGNORECASE)
                if match:
                    ref['url'] = f'https://cwe.mitre.org/data/definitions/{match.group(1)}.html'
                elif re.fullmatch(r'CVE-\d{4}-\d+', ref['id'], re.IGNORECASE):
                    ref['url'] = f'https://www.cve.org/CVERecord?id={ref["id"].upper()}'

        # The page only lists the actionable deviations.
        self.hardening_failures = sorted(
            (o for o in self.hardening['options'] if o['result'] == 'FAIL' and o['visible']),
            key=lambda o: o['name'],
        )
    ...
    def create_html(self):
        ...
        if self.hardening is not None:
            template = env.get_template("hardening.html")
            context = {'build': self, 'title': 'Hardening', 'current': 'hardening'}
            fname = os.path.join(self.output_path, 'hardening.html')
            logger.debug(f"Creating {fname}")
            with open(fname, 'w', encoding="utf-8") as fd:
                fd.write(template.render(**context))
...
def parse_args():
    ...
    parser.add_argument(
        "--hardening-report",
        help="Path to a hardening report JSON file generated by hardenconfig",
    )
    parser.add_argument(
