...
""",
    ...
    def check_hardening_data(self, kconf):
        ...
        exist, with values coherent with each symbol's type, and that CVEs
        mentioned in rationales are also listed as references.
        """
        ...
        for name, rule in database['rules'].items():
            ...
            for cve in re.findall(r"CVE-\d{4}-\d+", rule['rationale']):
                if cve not in rule['references']:
                    self.failure(
                        f"CONFIG_{name} in {rule['source']} mentions {cve} "
                        f"in its rationale but does not list it in its "
                        f"references. {hint}"
                    )
            ...
            elif sym.orig_type in (kconfiglib.INT, kconfiglib.HEX) and rule['value'] is not None:
                try:
                    int(rule['value'], 0)
                except ValueError:
                    self.failure(
