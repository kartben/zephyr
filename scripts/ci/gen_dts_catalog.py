...
files and ``runners.yaml`` files and produces a structured JSON catalog
that maps:

  - each board/target   → set of DT-compatible strings it uses
  - each board/target   → its west runners (available / flash / debug)
...

        {
          "targets": {
            "<board_target>": {
              ...
              "runners": {           # present only if a runners.yaml was found
                "available": ["pyocd", "jlink", ...],
                "flash":     "pyocd",
                "debug":     "pyocd"
              }
            }
...
def gather_board_build_info(twister_out_dir: Path) -> tuple[dict, dict]:
    """Load EDTs and runners info for every board target from a twister output directory.

    Args:
        twister_out_dir: Root of the twister output tree.

    Returns:
        A tuple ``(board_edts, board_runners)`` where:
          - ``board_edts`` is ``{board_name: {board_target: edt_object}}``.
          - ``board_runners`` is ``{board_name: {board_target: runners_yaml_dict}}``.
    """
    board_edts: dict = {}
    board_runners: dict = {}
    ...
    if not twister_out_dir.exists():
        ...
        return board_edts, board_runners
    ...
    for build_info_file in build_info_files:
        ...
        for runners_yaml_path in RUNNERS_YAML_PATHS:
            candidate = build_info_file.parent / runners_yaml_path
            if candidate.exists():
                runners_yaml_file = candidate
                break
        ...
        try:
            ...
            if runners_yaml_file:
                with open(runners_yaml_file) as fh:
                    runners_yaml = yaml.safe_load(fh)
...
def build_catalog(board_edts: dict, board_runners: dict | None = None) -> dict:
    ...
    Args:
        ...
        board_runners: Optional nested dict
            ``{board_name: {board_target: runners_yaml_dict}}`` of parsed
            ``runners.yaml`` files. When provided, each target entry in the
            catalog gains a ``runners`` field.
    ...
    boards_db = {}
    compatibles_db = {}
    board_runners = board_runners or {}

    for board_name, targets in board_edts.items():
        board_entry = {"targets": {}}
        target_runners = board_runners.get(board_name, {})
        ...
        for board_target, edt in targets.items():
            ...
            runners_yaml = target_runners.get(board_target)
            if runners_yaml is not None:
                target_entry["runners"] = {
                    "available": runners_yaml.get("runners", []) or [],
                    "flash": runners_yaml.get("flash-runner", "") or "",
                    "debug": runners_yaml.get("debug-runner", "") or "",
                }

            board_entry["targets"][board_target] = target_entry
...
def main(argv=None):
    ...
    try:
        ...
        logger.info("Harvesting EDT and runners data from %s …", twister_outdir)
        board_edts, board_runners = gather_board_build_info(twister_outdir)
        logger.info(
            "Loaded EDT data for %d boards (runners for %d)",
            len(board_edts),
            len(board_runners),
        )
        ...
        logger.info("Wrote %s  (%d boards, %d compatibles)", output_path, n_boards, n_compats)
