...
# 'source', 'rsource' and their optional variants. Relative forms resolve
# against the sourcing file's directory, plain ones against the tree root.
KCONFIG_SOURCE_RE = re.compile(r'^\s*(o?r?source)\s+"([^"]+)"')
...
def manifest_modules(manifest: Path, zephyr_base: Path) -> dict[str, ModuleAudit]:
    ...
    glue_directories = {
        sanitize(path.name): path.name
        for path in (zephyr_base / GLUE_ROOT).iterdir()
        if path.is_dir()
    }
...
def glue_owners(tree: dict[str, str], modules: dict[str, ModuleAudit]) -> dict[str, str]:
    ...
    Files in a module's glue directory belong to it as well. A module whose
    glue is split across several files only declares the symbol once, so a file
    that merely gates itself on one known presence symbol belongs to it too.
    """
    known = {sanitize(name) for name in modules}
    directories = {module.glue: module.name for module in modules.values() if module.glue}

    def named(module_symbols: set[str]) -> str | None:
        module_symbols &= known
        if len(module_symbols) != 1:
            return None
        return next(name for name in modules if sanitize(name) in module_symbols)
        ...
        owner = named(
            {
                match.group(1)
                for line in lines
                for match in [MODULE_DECLARATION_RE.match(line)]
                if match
            }
        )
        if owner is None:
            owner = next(
                (name for glue, name in directories.items() if path.startswith(glue)), None
            )
        if owner is None:
            owner = named(set(MODULE_SYMBOL_RE.findall(text)))
        if owner:
            owners[path] = owner
    return owners


def _inherited_gates(tree: dict[str, str], owners: dict[str, str]) -> dict[str, set[str]]:
    """Map each glue file to the symbols gating the 'source' that pulled it in."""
    gates: dict[str, set[str]] = defaultdict(set)
    for path in owners:
        directory = Path(path).parent
        for pattern, enclosing in _sourced_files(tree[path]):
            if not enclosing or "$" in pattern:
                continue
            for candidate in owners:
                relative = Path(candidate)
                if fnmatch(candidate, pattern) or fnmatch(
                    str(relative.relative_to(directory)) if directory in relative.parents else "",
                    pattern,
                ):
                    gates[candidate] |= enclosing
    return gates


def module_symbols(
    tree: dict[str, str], modules: dict[str, ModuleAudit]
    ...
    inherited = _inherited_gates(tree, owners)
    ...
    for owner, paths in by_owner.items():
        ...
        # One fixpoint over all of a module's glue: a gate may name a symbol
        # its sibling file declares, and a file sourced under a condition
        # passes that condition to everything it declares.
        declarations = [
            (symbol, gates | inherited[path])
            for path in paths
            for symbol, gates in _declarations(tree[path])
        ]
    ...
    # second declaration outside module glue cancels the gate unless it
    # repeats the dependency.
    for path, text in tree.items():
        if path in owners or not Path(path).name.startswith("Kconfig"):
            continue
        declarations = list(_declarations(text))
        redeclared = defaultdict(set)
        for symbol, _ in declarations:
            if symbol in symbols:
                redeclared[symbols[symbol][0]].add(symbol)
        for owner, owned in redeclared.items():
            presence = f"ZEPHYR_{sanitize(owner)}_MODULE"
            gated = _gated_by(declarations, {presence, f"{presence}_BLOBS"})
            for symbol in owned - gated:
    ...
    for module in modules.values():
        module.conditional_symbols = sorted(
            symbol
            for symbol, (owner, conditional) in symbols.items()
...
def _sourced_files(text: str):
    """Yield (path pattern, the symbols that gate it) for one Kconfig file.

    A 'source' inside an 'if' or a menu gates everything the sourced file
    declares, which is how a module keeps a whole SDK behind one symbol.
    """
    ...
    for raw in text.splitlines():
        ...
        if line.startswith(("if ", "if\t")):
            ...
            match = KCONFIG_SOURCE_RE.match(line)
            if match:
                yield match.group(2), set().union(*enclosing) if enclosing else set()
...
def _required_symbols(expression: str) -> set[str]:
    ...
    return {
        match.group(1)
        for match in KCONFIG_SYMBOL_RE.finditer(expression)
...
def unattributed_glue(
    ...
    which module the file integrates. A glue file that neither declares one nor
    gates itself on one cannot be tied to a module, so nothing it declares can
    be recognized as a dependency on one.
    """
    owners = glue_owners(tree, modules)
    unattributed = {}
    for path, text in tree.items():
        ...
        declared = [
            match.group(1)
            for line in lines
            for match in [KCONFIG_DECLARATION_RE.match(line)]
            if match
        ]
        if declared:
...
def print_table(modules: list[ModuleAudit], out=None) -> None:
    ...
    rows = [
        (
            ...
            str(len(m.consumers)),
            str(len(m.declared)),
            str(len(m.undeclared_consumers)),
            m.candidate,
        )
        for m in modules
    ]
    ...
    print(
        ...
        f"{sum(1 for m in modules if m.candidate == 'no')} with dependency debt",
        file=out,
    )
...
def print_details(module: ModuleAudit, out=None) -> None:
    ...
    if module.areas:
        print(
            "  used from:       "
            + ", ".join(f"{area} ({count})" for area, count in module.areas.items()),
            file=out,
        )
    if module.conditional_symbols:
        print(
            "  symbols that require the module: " + ", ".join(module.conditional_symbols), file=out
        )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        "--manifest",
        type=Path,
        help="manifest listing the candidate modules (default: <zephyr-base>/west.yml)",
    )
    parser.add_argument(
        "--module",
        action="append",
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        choices=("platform", "driver", "software", "bootstrap", "tooling", "no-evidence"),
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.unattributed_glue:
        for path, declared in sorted(unattributed_glue(read_tree(zephyr_base), modules).items()):
            print(
                f"{path} names no ZEPHYR_<MODULE>_MODULE symbol, so its "
    ...
    if args.format == "json":
        json.dump(
            {"schema_version": 1, "modules": [m.as_dict() for m in selected]}, sys.stdout, indent=2
        )
        print()
        ...
        for module in selected:
            writer.writerow(
                (
                    ...
                    len(module.consumers),
                    len(module.declared),
                    len(module.undeclared_consumers),
                    module.candidate,
                )
            )
    else:
