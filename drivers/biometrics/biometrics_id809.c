...
while (k_uptime_get() < deadline) {
ret = i2c_read_dt(&((const struct id809_config *)dev->config)->bus, &ch, sizeof(ch));
if (ret < 0) {
k_sleep(K_MSEC(ID809_I2C_POLL_MS));
continue;
}
...
read_len = (packet_kind == ID809_PACKET_KIND_RESPONSE) ?
   (ID809_FIXED_RESPONSE_DATA_SIZE + ID809_CKS_SIZE) : header.len;
...
static int id809_command(const struct device *dev, uint16_t cmd, const uint8_t *payload,
 uint16_t len, uint8_t *rsp_payload, size_t rsp_size,
...
data->enroll_state = (state == ID809_ENROLL_WAIT_SAMPLE_3) ? ID809_ENROLL_READY :
...
k_mutex_lock(&data->lock, K_FOREVER);
ret = id809_command(dev, ID809_CMD_GET_ENROLLED_ID_LIST, NULL, 0, rsp, sizeof(rsp), &rsp_len,
     ID809_RESPONSE_TIMEOUT_MS);
if (ret == -ENOENT) {
*actual_count = 0;
ret = 0;
...
ret = id809_command(dev, ID809_CMD_SEARCH, cmd, sizeof(cmd), rsp, sizeof(rsp), &rsp_len,
     ID809_RESPONSE_TIMEOUT_MS);
if (ret < 0) {
goto out;
}
...
result->template_id = (mode == BIOMETRIC_MATCH_IDENTIFY) ? data->last_match_id :
      template_id;
