...
static int auxdisplay_nhd_k3z_command(const struct device *dev, uint8_t command)
{
	...
	if (config->bus_type == AUXDISPLAY_NHD_K3Z_BUS_UART) {
		rc = auxdisplay_nhd_k3z_write_raw(dev, NHD_K3Z_UART_COMMAND_PREFIX,
						  &payload, sizeof(payload));
	} else {
		rc = auxdisplay_nhd_k3z_write_raw(dev, NHD_K3Z_CONTROL_COMMAND,
						  &payload, sizeof(payload));
	}

	k_sleep(K_USEC(config->command_delay_us));
	return rc;
}
						 ...
						 int16_t y)
{
	...
	return auxdisplay_nhd_k3z_command(dev, NHD_K3Z_CMD_SET_DDRAM_ADDR |
					      (xpos + auxdisplay_nhd_k3z_row_offset(ypos)));
}
...
static int auxdisplay_nhd_k3z_clear(const struct device *dev)
{
	...
	k_sleep(K_MSEC(config->clear_delay_ms));
	...
	for (uint8_t row = 0; row < NHD_K3Z_CUSTOM_CHARACTER_HEIGHT; row++) {
		...
		for (uint8_t col = 0; col < NHD_K3Z_CUSTOM_CHARACTER_WIDTH; col++) {
			if (character->data[(row * NHD_K3Z_CUSTOM_CHARACTER_WIDTH) + col] > 0) {
				character_data[row] |= BIT(NHD_K3Z_CUSTOM_CHARACTER_WIDTH - col - 1);
			}
		}
	}
...
#define AUXDISPLAY_NHD_K3Z_INIT(inst)                                                         \
	...
	static const struct auxdisplay_nhd_k3z_config auxdisplay_nhd_k3z_config_##inst = {      \
		...
		.command_delay_us = DT_INST_PROP_OR(inst, command_delay_us, 40),                \
		.clear_delay_ms = DT_INST_PROP_OR(inst, clear_delay_ms, 2),                     \
		AUXDISPLAY_NHD_K3Z_CONFIG_BUS(inst),                                             \
	};                                                                                      \
                                                                                               \
	DEVICE_DT_INST_DEFINE(inst, auxdisplay_nhd_k3z_init, NULL,                              \
			      &auxdisplay_nhd_k3z_data_##inst,                                  \
			      &auxdisplay_nhd_k3z_config_##inst, POST_KERNEL,                   \
			      CONFIG_AUXDISPLAY_INIT_PRIORITY, &auxdisplay_nhd_k3z_api);
