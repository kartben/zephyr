...
extern void gpio_keys_interrupt(const struct device *port, struct gpio_callback *cb, uint32_t pins);
extern int gpio_keys_pm_action(const struct device *dev, enum pm_device_action action);
...
const struct gpio_keys_pin_config *gpio_keys_acc_pin_cfg(const struct gpio_keys_config *cfg, int i)
{
	return &cfg->pin_cfg[i];
}

struct gpio_keys_pin_data *gpio_keys_acc_pin_data(const struct gpio_keys_config *cfg, int i)
{
...
const struct gpio_dt_spec *gpio_keys_acc_pin_cfg_spec(const struct gpio_keys_pin_config *pin_cfg)
{
	return &pin_cfg->spec;
}
...
const struct device *gpio_keys_acc_pin_data_dev(const struct gpio_keys_pin_data *pin_data)
{
	return pin_data->dev;
}

void gpio_keys_acc_pin_data_set_dev(struct gpio_keys_pin_data *pin_data, const struct device *dev)
{
	pin_data->dev = dev;
}
...
void gpio_keys_acc_set_pin_state(struct gpio_keys_pin_data *pin_data, int state)
{
	pin_data->cb_data.pin_state = (int8_t)state;
}

struct gpio_keys_callback *gpio_keys_acc_pin_data_cb(struct gpio_keys_pin_data *pin_data)
{
	return &pin_data->cb_data;
}

struct k_work_delayable *gpio_keys_acc_pin_data_work(struct gpio_keys_pin_data *pin_data)
{
...
struct gpio_keys_callback *gpio_keys_acc_keys_cb_from_cb(struct gpio_callback *cbdata)
{
	return CONTAINER_OF(cbdata, struct gpio_keys_callback, gpio_cb);
}

struct gpio_keys_pin_data *gpio_keys_acc_cb_to_pin_data(struct gpio_keys_callback *keys_cb)
{
...
void gpio_keys_acc_work_init(struct gpio_keys_pin_data *pin_data, int is_polling)
{
	k_work_handler_t handler = is_polling ? gpio_keys_poll_pins : gpio_keys_change_deferred;

	k_work_init_delayable(&pin_data->work, handler);
}

void gpio_keys_acc_work_reschedule(struct k_work_delayable *dwork, uint32_t ms)
{
...
void gpio_keys_acc_report_key(const struct device *dev, uint32_t code, int pressed)
{
...
void gpio_keys_log_dbg_poll(const char *name, int pin_state, int new_pressed, int key_index)
{
	LOG_DBG("%s: pin_state=%d, new_pressed=%d, key_index=%d", name, pin_state, new_pressed,
		key_index);
}

void gpio_keys_log_dbg_report(const char *name, int new_pressed, uint32_t code)
{
...
static int gpio_keys_init(const struct device *dev)
{
	...
	for (int i = 0; i < cfg->num_keys; i++) {
		...
		ret = gpio_keys_interrupt_configure(&cfg->pin_cfg[i].spec, &pin_data[i].cb_data);
		if (ret != 0) {
			LOG_ERR("Pin %d interrupt configuration failed: %d", i, ret);
			return ret;
		}
...
#ifdef CONFIG_PM_DEVICE
static int gpio_keys_pm_action(const struct device *dev, enum pm_device_action action)
{
	const struct gpio_keys_config *cfg = dev->config;
	struct gpio_keys_data *data = dev->data;
	struct gpio_keys_pin_data *pin_data = cfg->pin_data;
...
#define GPIO_KEYS_INIT(i)                                                                          \
	...
	static const struct gpio_keys_config gpio_keys_config_##i = {                              \
		...
		.handler = COND_CODE_1(DT_INST_PROP(i, polling_mode), (gpio_keys_poll_pins),            \
