...
#define SI12T_THRESHOLD1_ADDR   0x02
#define SI12T_THRESHOLD2_ADDR   0x03
#define SI12T_THRESHOLD3_ADDR   0x04
#define SI12T_THRESHOLD4_ADDR   0x05
#define SI12T_THRESHOLD5_ADDR   0x06
#define SI12T_THRESHOLD6_ADDR   0x07
...
struct si12t_config {
	...
	uint8_t thresholds[SI12T_NUM_CHANNELS];
	uint8_t num_codes;
};
...
static int si12t_set_thresholds(const struct device *dev)
{
	const struct si12t_config *config = dev->config;

	/* Set thresholds for all 6 registers (covering 12 channels) */
	/* Each register covers 2 channels (4 bits each) */
	for (uint8_t i = 0; i < 6; i++) {
		uint8_t reg = SI12T_THRESHOLD1_ADDR + i;
		uint8_t ch_even = i * 2;
		uint8_t ch_odd = i * 2 + 1;
		uint8_t val = (config->thresholds[ch_even] << 4) | config->thresholds[ch_odd];
...
static int si12t_init_chip(const struct device *dev)
{
	...
	return si12t_set_thresholds(dev);
}
...
#define SI12T_WINDOW_NARROW 0
#define SI12T_WINDOW_WIDE   1

#define SI12T_CHANNEL_THRESHOLD(i, n) \
	((UTIL_CAT(SI12T_WINDOW_, DT_INST_STRING_UPPER_TOKEN_BY_IDX(n, threshold_windows, i)) \
	  ? 0x08 : 0) | DT_INST_PROP_BY_IDX(n, threshold_midpoints, i))

#define SI12T_INIT(n)                                                                              \
	BUILD_ASSERT(DT_INST_PROP_LEN(n, input_codes) > 0,                                         \
		     "input-codes must have at least one entry");                                  \
	BUILD_ASSERT(DT_INST_PROP_LEN(n, input_codes) <= SI12T_NUM_CHANNELS,                       \
		     "input-codes must not exceed SI12T_NUM_CHANNELS");                            \
	...
	static const struct si12t_config si12t_config_##n = {                                      \
		...
		.thresholds = {                                                                    \
			LISTIFY(DT_INST_PROP_LEN(n, input_codes), SI12T_CHANNEL_THRESHOLD, (,), n) \
		},                                                                                 \
