...
#define RENESAS_RA_GLCDC_IN_PIXEL_FORMAT(n)                                                        \
	CONCAT(RENESAS_RA_GLCDC_IN_PIXEL_FORMAT_, INPUT_FORMAT_PIXEL(n))

#define RENESAS_RA_GLCDC_OUT_PIXEL_FORMAT(n)                                                       \
	CONCAT(RENESAS_RA_GLCDC_OUT_PIXEL_FORMAT_, OUTPUT_FORMAT_PIXEL(n))
...
#define RENESAS_RA_GLCDC_OUTPUT_ENDIAN(n)                                                          \
	CONCAT(DISPLAY_ENDIAN_, DT_INST_STRING_UPPER_TOKEN_OR(n, output_endian, LITTLE))

#define RENESAS_RA_GLCDC_OUTPUT_COLOR_ODER(n)                                                      \
	CONCAT(DISPLAY_COLOR_ORDER_, DT_INST_STRING_UPPER_TOKEN_OR(n, output_color_oder, RGB))

#define RENESAS_RA_GLCDC_OUTPUT_DE_POLARITY(n)                                                     \
	CONCAT(DISPLAY_SIGNAL_POLARITY_,                                                           \
		 DT_INST_STRING_UPPER_TOKEN_OR(n, output_data_signal_polarity, HIACTIVE))

#define RENESAS_RA_GLCDC_OUTPUT_SYNC_EDGE(n)                                                       \
	CONCAT(DISPLAY_SIGNAL_SYNC_EDGE_,                                                          \
...
#define RENESAS_RA_GLCDC_TCON_HSYNC_PIN(n)                                                         \
	CONCAT(GLCDC_, DT_INST_STRING_UPPER_TOKEN_OR(n, output_pin_hsync, TCON_PIN_1))

#define RENESAS_RA_GLCDC_TCON_VSYNC_PIN(n)                                                         \
	CONCAT(GLCDC_, DT_INST_STRING_UPPER_TOKEN_OR(n, output_pin_vsync, TCON_PIN_0))

#define RENESAS_RA_GLCDC_TCON_DE_PIN(n)                                                            \
	CONCAT(GLCDC_, DT_INST_STRING_UPPER_TOKEN_OR(n, output_pin_de, TCON_PIN_2))

#define RENESAS_RA_GLCDC_OUTPUT_CLOCK_DIV(n)                                                       \
	CONCAT(GLCDC_PANEL_CLK_DIVISOR_, DT_INST_PROP_OR(n, output_clock_divisor, 8))
