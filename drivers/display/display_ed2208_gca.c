			    ...
			    const struct display_buffer_descriptor *desc, const void *buf)
{
	...
	ret = display_color_dither_prepare(dev, DISPLAY_COLOR_DITHER_STATE(data), &desc, &buf,
					   &scratch);
	if (ret < 0) {
		return ret;
	}
...
#define ED2208_GCA_DEFINE(inst)                                                                    \
	...
	static struct ed2208_gca_data ed2208_gca_data_##inst = {IF_ENABLED(CONFIG_DISPLAY_COLOR_DITHER,                                           \
