...
static int sdl_display_init(const struct device *dev)
{
	...
	if (display_color_dither_claim(DISPLAY_COLOR_DITHER_STATE(disp_data),
				       disp_data->current_pixel_format) == 0) {
		disp_data->current_pixel_format = PIXEL_FORMAT_I_4;
	} else {
		display_color_dither_passthrough(DISPLAY_COLOR_DITHER_STATE(disp_data));
	}
			     ...
			     const void *buf)
{
	...
	ret = display_color_dither_prepare(dev, DISPLAY_COLOR_DITHER_STATE(disp_data), &desc, &buf,
					   &scratch);
	if (ret < 0) {
		return ret;
	}
	...
	const struct device *dev, struct display_capabilities *capabilities)
{
	...
	display_color_dither_patch_caps(DISPLAY_COLOR_DITHER_STATE(disp_data), capabilities);
}
					...
					const enum display_pixel_format pixel_format)
{
	...
	if (display_color_dither_claim(DISPLAY_COLOR_DITHER_STATE(disp_data), pixel_format) == 0) {
		disp_data->current_pixel_format = PIXEL_FORMAT_I_4;
		return 0;
	}
