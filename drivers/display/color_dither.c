...
static int find_nearest_palette_idx(const struct display_palette_color *palette, uint8_t r,
				    uint8_t g, uint8_t b)
{
...
static int quantize_to_i4(const struct display_palette_color *palette, uint8_t r, uint8_t g,
			  uint8_t b, uint8_t *converted_buf, size_t row_size, uint32_t x,
			  uint32_t y)
{
	int palette_idx = find_nearest_palette_idx(palette, r, g, b);
				   ...
				   const struct color_dither_errdiff_kernel *kernel)
{
	...
	for (uint32_t y = 0U; y < desc->height; y++) {
		...
		for (uint32_t x = 0U; x < desc->width; x++) {
			...
			r = error_apply(r, current_row[x].r, kernel->divisor);
			g = error_apply(g, current_row[x].g, kernel->divisor);
			b = error_apply(b, current_row[x].b, kernel->divisor);
			palette_idx = quantize_to_i4(palette, r, g, b, state->converted_buf,
						     row_size, x, y);
			if (palette_idx < 0) {
				return -ENOTSUP;
			}
			   ...
			   enum color_dither_algorithm algorithm)
{
	...
	for (uint32_t y = 0U; y < desc->height; y++) {
		for (uint32_t x = 0U; x < desc->width; x++) {
			...
			if (quantize_to_i4(palette, r, g, b, converted_buf, row_size, x, y) < 0) {
				return -ENOTSUP;
			}
		}
	}
...
static uint32_t display_color_dither_supported_formats(void)
{
	uint32_t formats = 0U;

	if (IS_ENABLED(CONFIG_DISPLAY_COLOR_DITHER_INPUT_RGB565)) {
		formats |= PIXEL_FORMAT_RGB_565;
	}
	if (IS_ENABLED(CONFIG_DISPLAY_COLOR_DITHER_INPUT_RGB888)) {
			       ...
			       enum display_pixel_format pf)
{
	...
	if (!IS_POWER_OF_TWO(pf) || (pf & display_color_dither_supported_formats()) == 0U) {
		return -ENOTSUP;
	}
				     ...
				     struct display_capabilities *caps)
{
	...
	caps->supported_pixel_formats |= display_color_dither_supported_formats();
...
{
	if (state == NULL || state->converted_buf == NULL ||
	    state->current_format == PIXEL_FORMAT_I_4) {
		return 0;
	}
	...
	case COLOR_DITHER_ALGO_BAYER_8X8:
		ret = convert_ordered(in_desc, state->current_format, *buf_in_out,
				      caps.color_palette, state->converted_buf,
				      COLOR_DITHER_ACTIVE_ALGORITHM);
		break;
#ifdef CONFIG_DISPLAY_COLOR_DITHER_ALGORITHM_FLOYD_STEINBERG
	case COLOR_DITHER_ALGO_FLOYD_STEINBERG:
		ret = convert_error_diffusion(in_desc, state->current_format, *buf_in_out,
...
{
	(void)display_color_dither_claim(state, PIXEL_FORMAT_I_4);
}
