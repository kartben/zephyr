...
static int quantize_to_i4(const struct display_palette_color *palette, size_t palette_len,
...
static int convert_error_diffusion(const struct display_buffer_descriptor *desc,
				   ...
				   struct display_color_dither_rgb_error *const state_rows[3],
				   size_t row_len, int divisor,
				   const struct color_dither_errdiff_tap *taps, size_t n_taps,
				   size_t n_rows)
{
	size_t row_size = DIV_ROUND_UP(desc->width, 2U);
	struct display_color_dither_rgb_error *rows[3] = {
		state_rows[0],
		state_rows[1],
		state_rows[2],
	};
	...
	for (uint32_t y = 0U; y < desc->height; y++) {
		for (uint32_t x = 0U; x < desc->width; x++) {
			...
			palette_idx = quantize_to_i4(palette, palette_len, r, g, b, converted_buf,
						     row_size, x, y);
			if (palette_idx < 0) {
				return -ENOTSUP;
			}
		 ...
		 */
		struct display_color_dither_rgb_error *tmp = rows[0];
...
static int convert_ordered(const struct display_buffer_descriptor *desc,
			   enum display_pixel_format pixel_format, const void *buf,
			   const struct display_palette_color *palette, size_t palette_len,
			   uint8_t *converted_buf, enum color_dither_algorithm algorithm)
{
	size_t row_size = DIV_ROUND_UP(desc->width, 2U);
	...
	for (uint32_t y = 0U; y < desc->height; y++) {
		for (uint32_t x = 0U; x < desc->width; x++) {
			...
			if (quantize_to_i4(palette, palette_len, r, g, b, converted_buf, row_size,
					   x, y) < 0) {
				return -ENOTSUP;
			}
		}
