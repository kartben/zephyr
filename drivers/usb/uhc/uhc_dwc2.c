...
static int dwc2_set_fifo_sizes(struct usb_dwc2_reg *const base)
{
	...
	/*
	 * The receive FIFO is shared by every endpoint, so it has to absorb the
	 * largest packet the port can receive back to back.
	 */
...
static uint32_t ch_handle_in_bulk_control(struct uhc_dwc2_channel *ch, uint32_t hcint)
{
	...
	if (hcint & USB_DWC2_HCINT_CHHLTD) {
		if (hcint & (USB_DWC2_HCINT_XFERCOMPL | USB_DWC2_HCINT_STALL |
			...
			/*
			 * The channel is halted and will not restart on its
			 * own, so fail the transfer instead of leaving the
			 * channel claimed forever.
			 */
 ...
 * hardware. Control transfers derive their PID from the stage instead.
 */
...
/*
 * Restart a halted bulk transfer where the wire left off instead of from the
 * beginning. PKTCNT is only decremented for packets that completed their
 * transaction, and every packet but the last one of a transfer is maximum
 * sized, so it gives the amount of data the device has already seen.
 */
