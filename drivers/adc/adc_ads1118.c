...
static int ads1118_configure_gain(const struct device *dev,
				  ...
				  enum ads1118_config_pga *config_pga)
{
	ARG_UNUSED(dev);

	switch (channel_cfg->gain) {
	case ADC_GAIN_2_3:
		*config_pga = ADS1118_CONFIG_PGA_6144;
		break;
	case ADC_GAIN_1:
		*config_pga = ADS1118_CONFIG_PGA_4096;
		break;
	case ADC_GAIN_2:
		*config_pga = ADS1118_CONFIG_PGA_2048;
		break;
	case ADC_GAIN_4:
		*config_pga = ADS1118_CONFIG_PGA_1024;
		break;
	case ADC_GAIN_8:
		*config_pga = ADS1118_CONFIG_PGA_512;
		break;
	case ADC_GAIN_16:
		*config_pga = ADS1118_CONFIG_PGA_256;
		break;
	default:
		return -EINVAL;
	}
...
static int ads1118_acq_time_to_dr(const struct device *dev, uint16_t acq_time,
				  enum ads1118_config_dr *config_dr)
{
...
static int ads1118_configure_multiplexer(const struct device *dev,
					 const struct adc_channel_cfg *channel_cfg,
					 enum ads1118_config_mux *config_mux)
{
	...
	if (ads_config->multiplexer) {
		...
		if (channel_cfg->differential) {
			if (channel_cfg->input_positive == 0 && channel_cfg->input_negative == 1) {
				...
				*config_mux = ADS1118_CONFIG_MUX_DIFF_0_3;
			} else if (channel_cfg->input_positive == 1 &&
				   channel_cfg->input_negative == 3) {
				*config_mux = ADS1118_CONFIG_MUX_DIFF_1_3;
			} else if (channel_cfg->input_positive == 2 &&
				   channel_cfg->input_negative == 3) {
				*config_mux = ADS1118_CONFIG_MUX_DIFF_2_3;
			} else {
			...
			if (channel_cfg->input_positive <= 3) {
				*config_mux =
					ADS1118_CONFIG_MUX_SINGLE_0 + channel_cfg->input_positive;
			} else {
...
static int ads1118_setup(const struct device *dev, const struct adc_channel_cfg *channel_cfg)
{
	...
	ret = ads1118_configure_multiplexer(dev, channel_cfg, &config_mux);
	if (ret != 0) {
		return ret;
	}
	config |= ADS1118_CONFIG_MUX(config_mux);

	ret = ads1118_configure_gain(dev, channel_cfg, &config_pga);
	if (ret != 0) {
		return ret;
	}
	config |= ADS1118_CONFIG_PGA(config_pga);

	ret = ads1118_acq_time_to_dr(dev, channel_cfg->acquisition_time, &config_dr);
	if (ret != 0) {
		return ret;
	}
	config |= ADS1118_CONFIG_DR(config_dr);
