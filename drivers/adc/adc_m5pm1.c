...
static int adc_m5pm1_sample(const struct device *dev, uint8_t channel, uint16_t *value)
{
	...
	case ADC_M5PM1_CHANNEL_GPIO1:
		ret = adc_m5pm1_route_gpio(config->mfd, M5PM1_ADC_HW_CH_GPIO1);
		if (ret < 0) {
			return ret;
		}
		return adc_m5pm1_convert(dev, M5PM1_ADC_HW_CH_GPIO1, value);
	case ADC_M5PM1_CHANNEL_GPIO2:
		ret = adc_m5pm1_route_gpio(config->mfd, M5PM1_ADC_HW_CH_GPIO2);
		if (ret < 0) {
			return ret;
		}
...
/* PMIC registers report millivolts; with 12-bit resolution this yields µV. */
