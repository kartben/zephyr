...
static int rx_stream_start(struct stream *stream, const struct device *dev)
{
	...
	if (cfg->ext_rx) {
		/*
		 * The extension block is always a slave, so the main block has to
		 * drive the bus. A master transmitter only clocks while it has data
		 * to send, so when nothing transmits the main block is made a master
		 * receiver instead, whose clocks free-run as long as it is enabled.
		 */
		uint32_t main_mode;
