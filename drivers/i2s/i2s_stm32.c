...
/*
 * Whether a stream still drives the bus. With i2s-ext-rx both directions
...
static int rx_stream_start(struct stream *stream, const struct device *dev)
{
	...
	if (cfg->ext_rx) {
		...
		/*
		 * The extension block never drives the clocks, so the main
		 * block has to. A transmitting controller only clocks while it
		 * has data to send, so when nothing transmits the main block is
		 * made a receiver instead, whose clocks free-run as long as it
		 * is enabled. Its data pin then floats, which only costs a
		 * permanent overrun nothing reads.
		 */
		if (!stream->master) {
			main_mode = LL_I2S_MODE_SLAVE_TX;
		} else if (stream_is_active(&dev_data->tx)) {
			main_mode = LL_I2S_MODE_MASTER_TX;
		} else {
			main_mode = LL_I2S_MODE_MASTER_RX;
		}

		LL_I2S_SetTransferMode(cfg->i2s_rx, LL_I2S_MODE_SLAVE_RX);
		LL_I2S_SetTransferMode(cfg->i2s, main_mode);
...
static int tx_stream_start(struct stream *stream, const struct device *dev)
{
	...
	/*
	 * The main block may already be clocking a receive on the extension
	 * block, and its direction only takes while it is disabled.
	 */
	if (cfg->ext_rx) {
...
static void tx_stream_disable(struct stream *stream, const struct device *dev)
{
	...
	if (cfg->ext_rx && stream_is_active(&dev_data->rx)) {
		/*
		 * A receive on the extension block still needs the clocks, and
		 * a transmitter with nothing left to send would stop them.
		 */
...
#define I2S_STM32_EXT_RX(index) DT_INST_PROP_OR(index, i2s_ext_rx, 0)
