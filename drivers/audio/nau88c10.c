...
#define NAU88C10_AUDIO_IF_AIFMT_RIGHT_JUSTIFIED (0x0 << 3)
#define NAU88C10_AUDIO_IF_AIFMT_LEFT_JUSTIFIED  (0x1 << 3)
#define NAU88C10_AUDIO_IF_AIFMT_I2S             (0x2 << 3)
#define NAU88C10_AUDIO_IF_WLEN_16               (0x0 << 5)
#define NAU88C10_AUDIO_IF_WLEN_20               (0x1 << 5)
#define NAU88C10_AUDIO_IF_WLEN_24               (0x2 << 5)
#define NAU88C10_AUDIO_IF_WLEN_32               (0x3 << 5)
...
#define NAU88C10_CLOCK1_MCLKSEL(sel) ((sel) << 5)

/* Clock Control 2 (0x07) */
#define NAU88C10_CLOCK2_SMPLR(rate) ((rate) << 1)
...
/*
 * A register write is a 7-bit register address followed by a 9-bit value, both
...
static int nau88c10_write(const struct device *dev, uint8_t reg, uint16_t val)
{
	...
	uint8_t buf[2] = {(reg << 1) | ((val >> 8) & 0x1), val & 0xff};
...
static int nau88c10_read(const struct device *dev, uint8_t reg, uint16_t *val)
{
	...
	*val = ((uint16_t)(buf[0] & 0x1) << 8) | buf[1];
...
static int nau88c10_audio_if(const struct audio_codec_cfg *codec_cfg, uint16_t *audio_if)
{
	...
	case AUDIO_DAI_TYPE_I2S:
		*audio_if = NAU88C10_AUDIO_IF_AIFMT_I2S;
		break;
	case AUDIO_DAI_TYPE_LEFT_JUSTIFIED:
		*audio_if = NAU88C10_AUDIO_IF_AIFMT_LEFT_JUSTIFIED;
		break;
	case AUDIO_DAI_TYPE_RIGHT_JUSTIFIED:
		*audio_if = NAU88C10_AUDIO_IF_AIFMT_RIGHT_JUSTIFIED;
		break;
	default:
	...
	case AUDIO_PCM_WIDTH_16_BITS:
		*audio_if |= NAU88C10_AUDIO_IF_WLEN_16;
		break;
	case AUDIO_PCM_WIDTH_20_BITS:
		*audio_if |= NAU88C10_AUDIO_IF_WLEN_20;
		break;
	case AUDIO_PCM_WIDTH_24_BITS:
		*audio_if |= NAU88C10_AUDIO_IF_WLEN_24;
		break;
	case AUDIO_PCM_WIDTH_32_BITS:
		*audio_if |= NAU88C10_AUDIO_IF_WLEN_32;
		break;
	default:
...
static int nau88c10_configure(const struct device *dev, struct audio_codec_cfg *codec_cfg)
{
	...
	ret = nau88c10_write(dev, NAU88C10_REG_CLOCK1, NAU88C10_CLOCK1_MCLKSEL(mclk_sel));
	if (ret < 0) {
		return ret;
	}

	ret = nau88c10_write(dev, NAU88C10_REG_CLOCK2, NAU88C10_CLOCK2_SMPLR(rate_sel));
	if (ret < 0) {
		return ret;
	}
...
static int nau88c10_set_output_volume(const struct device *dev, int vol)
{
	...
	if (vol < NAU88C10_SPK_VOLUME_MIN_DB || vol > NAU88C10_SPK_VOLUME_MAX_DB) {
...
static int nau88c10_set_input_volume(const struct device *dev, int vol)
{
	...
	if (vol < NAU88C10_PGA_GAIN_MIN_DB || vol > NAU88C10_PGA_GAIN_MAX_DB) {
...
#define NAU88C10_INIT(inst)                                                                        \
	static const struct nau88c10_config nau88c10_config_##inst = {                             \
		...
		.mic_single_ended = DT_INST_PROP(inst, nuvoton_mic_single_ended),                  \
	};                                                                                         \
                                                                                                   \
