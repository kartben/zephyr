...
static const char *ns_dmic_path_get(const struct ns_dmic_config *cfg,
				    const struct ns_dmic_data *data)
{
	if ((data != NULL) && (data->cmd_file_path != NULL)) {
		return data->cmd_file_path;
	}
...
static void ns_dmic_pace(struct ns_dmic_data *data, size_t size)
{
	...
	__ASSERT_NO_MSG(bytes_per_sec != 0U);
...
static int ns_dmic_open_file(const struct ns_dmic_config *cfg, struct ns_dmic_data *data,
			     bool reset_deadline)
{
	const char *path = ns_dmic_path_get(cfg, data);
	int fd;
				...
				struct dmic_cfg *config)
{
	...
	path = ns_dmic_path_get(cfg, data);
	if ((path == NULL) || (path[0] == '\0')) {
		return -EINVAL;
	}
...
#define NS_DMIC_COMMAND_LINE_OPTS(inst)                                                            \
	{                                                                                          \
		...
		.name = "path",                                                                    \
		.type = 's',                                                                       \
		.dest = (void *)&ns_dmic_data_##inst.cmd_file_path,                                \
...
static void ns_dmic_options(void)
{
	static struct args_struct_t ns_dmic_options[] = {
		DT_INST_FOREACH_STATUS_OKAY(NS_DMIC_COMMAND_LINE_OPTS) ARG_TABLE_ENDMARKER,
	};
