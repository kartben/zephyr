...
static int dmic_esp32_configure(const struct device *dev, struct dmic_cfg *config)
{
	...
	if ((stream->pcm_width != 16U) || (stream->block_size == 0U) ||
	    ((stream->block_size % ((stream->pcm_width / 8U) *
				    config->channel.req_num_chan)) != 0U) ||
	...
	/* Keep I2S reads non-blocking; DMIC read() applies API-level timeout. */
	i2s_cfg.timeout = 0;
			   ...
			   void **buffer, size_t *size, int32_t timeout)
{
	...
	do {
		ret = i2s_read(cfg->i2s_dev, buffer, size);
		if (ret == 0) {
			return 0;
		}
		...
		if (timeout > 0 && k_uptime_get() >= end) {
			return -EAGAIN;
		}

		k_sleep(K_MSEC(1));
	} while (timeout == SYS_FOREVER_MS || timeout > 0);
