...
/*
 * Meter works in dBFS (0 dBFS = full-scale PCM). Both widths are normalized to a common Q15 full
 * scale so the two share one path: FS_N = 2^15 (amplitude), FS_SQ = 2^30 (power, the 0 dBFS ref).
 */
...
/* Per-channel measurements for one block, from PCM normalized to Q15 full scale (see FS_N). */
struct dmic_vu_meas {
...
/*
 * Power level in tenths of a dBFS: 10*log10(power / FS_SQ), <= 0 since power never exceeds full
 * scale. Returns INT32_MIN ("-inf") for digital silence.
 */
...
/*
 * Analyse one interleaved channel: peak amplitude and clip count, with samples normalized to Q15
 * full scale (which also sidesteps INT16_MIN/INT32_MIN negation UB, since +FS_N fits in int32_t).
 */
			    ...
			    uint8_t channel, struct dmic_vu_meas *m)
{
	...
	if (pcm_width == 16U) {
		...
		for (size_t i = channel; i < num_samples; i += num_channels) {
			int32_t x = (int32_t)(s[i] >> 16); /* normalize to Q15 */
			int32_t mag = (x < 0) ? -x : x;
...
static int cmd_read(const struct shell *sh, size_t argc, char *argv[])
{
	...
	for (uint32_t i = 0; i < count; i++) {
		...
		for (uint8_t ch = 0; ch < channels; ch++) {
			...
			analyze_channel(buf, size, pcm_width, channels, ch, &m);
			pct = (m.peak_n * 100 + DMIC_SHELL_VU_FS_N / 2) / DMIC_SHELL_VU_FS_N;
...
static int cmd_vu(const struct shell *sh, size_t argc, char *argv[])
{
	...
	/* Size the bar to the terminal width (capped for readout alignment on wide terminals). */
