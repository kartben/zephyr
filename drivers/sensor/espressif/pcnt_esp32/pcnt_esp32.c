...
int pcnt_esp32_init(const struct device *dev)
{
	...
	/*
	 * The limit events only reach the CPU once the peripheral interrupt
	 * enable is set. Arm it after esp_intr_alloc() so a failed allocation
	 * never leaves the peripheral asserting with no handler installed.
	 */
	for (uint8_t i = 0; i < config->unit_len; i++) {
