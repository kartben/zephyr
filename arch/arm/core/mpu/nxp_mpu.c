...
/* @brief Partition validation
 *
 * This internal function performs run-time validation for
 ...
 * validation of the memory regions to be programmed.
 */
				 ...
				 bool do_validation)
{
	...
	for (i = 0; i < regions_num; i++) {
		...
		if (do_validation &&
			...
			LOG_ERR("Partition %u: validation failed.", i);
			return -EINVAL;
		}
