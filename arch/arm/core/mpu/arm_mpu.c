 ...
 * validation check of the memory regions to be programmed.
 */
static int mpu_configure_regions(const struct z_arm_mpu_partition
	regions[], uint8_t regions_num, uint8_t start_reg_index,
	bool do_validation_check)
{
	int i;
	...
	for (i = 0; i < regions_num; i++) {
		...
		if (do_validation_check &&
			...
			LOG_ERR("Partition %u: validation check failed.", i);
			return -EINVAL;
		}
...
#pragma message "Null-Pointer exception detection cannot be configured on un-mapped flash areas"
	...
	/* Validation check for number of regions in Cortex-M0+, M3, and M4. */
#if defined(CONFIG_CPU_CORTEX_M0PLUS) || \
	defined(CONFIG_CPU_CORTEX_M3) || \
	defined(CONFIG_CPU_CORTEX_M4)
	__ASSERT(
