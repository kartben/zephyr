...
class TestPackageProvenance:
    ...
    def test_zephyr_sources_supplier_and_purl(self, zephyr_doc, zephyr_meta_remote):
        ...
        # the supplier is derived from that URL, so its value depends on the workspace
        assert get_supplier_name(pkg), "zephyr.spdx: zephyr-sources has no supplier"
        # the purl is built from that URL too, so only its shape can be asserted here
        purls = get_purl_refs(pkg)
        assert any("@" in p for p in purls), (
            f"zephyr.spdx: zephyr-sources has no revision-pinned purl, got {purls}"
        )
    ...
    def test_zephyr_deps_supplier_and_purl(self, modules_doc, zephyr_meta_remote):
        ...
        assert get_supplier_name(pkg), "modules-deps.spdx: zephyr-deps has no supplier"
        if not zephyr_meta_remote:
            pytest.skip("zephyr.meta has no remote URL for zephyr")
        purls = get_purl_refs(pkg)
        assert any("@" in p for p in purls), (
