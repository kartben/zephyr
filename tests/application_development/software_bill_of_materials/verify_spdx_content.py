...
# the used module's library is named after its in-tree path, so it is matched on
# its tail rather than spelled out in full
SUFFIX_USED_MODULE_LIB = "used_module.a"
...
# Fake vendor blobs provided by the used module, generated by its CMakeLists.
# Three are linked: as an imported library, via -lNAME/-L linker flags and via
# an -l:FILENAME flag. The -l:FILENAME one is deliberately not named lib*.a,
# since that is the whole point of that linker flag. An empty ar archive is
# just its 8-byte magic, which is all these three contain.
...
# The other two never reach the link line at all: they are embedded in the
# image, as a generated C array and through an .incbin directive.
FILE_BLOB_GENERATED = "./zephyr/blobs/sbom_blob_firmware.bin"
FILE_BLOB_INCBIN = "./zephyr/blobs/sbom_blob_incbin.bin"
# the .inc the build renders FILE_BLOB_GENERATED into; the SBOM must record the
# blob it was rendered from, not this build artifact
FILE_BLOB_GENERATED_INC = "sbom_blob_firmware.bin.inc"
BLOB_ARCHIVE_CONTENT = b"!<arch>\n"
BLOB_CONTENTS = {
    FILE_BLOB_IMPORTED: BLOB_ARCHIVE_CONTENT,
    FILE_BLOB_FLAG: BLOB_ARCHIVE_CONTENT,
    FILE_BLOB_LITERAL: BLOB_ARCHIVE_CONTENT,
    FILE_BLOB_GENERATED: b"sbom firmware blob\n",
    FILE_BLOB_INCBIN: b"sbom incbin blob\n",
}
BLOB_VERSION = "1.2.3"
...
def image_relationship_targets(build_doc, rel_type):
    """Return the targets of the final image's relationships of a given type."""
    ...
    return {
        ...
        for r in get_relationships_for_element(build_doc, elf.spdx_id, rel_type)
    }
...
class TestBlobs:
    """Tests for the vendor blobs that end up in the final image.

    The used module provides five fake blobs, consumed the ways real HAL modules
    consume theirs: linked as an imported library (hal_stm32 style), as
    -lNAME/-L linker flags (hal_espressif style) and as an -l:FILENAME flag
    naming the archive verbatim (the way hal_stm32 links the STM32WB0 BLE
    stack); and embedded without ever reaching the link line, by
    generate_inc_file_for_target() (the way the nRF70 and RW61x Wi-Fi firmware
    are) and by an .incbin directive pointed at a compile definition (the way
    the AIROC Wi-Fi firmware is). All five must show up in the SBOM.
    """

    @pytest.fixture(params=list(BLOB_CONTENTS))
    ...
    def test_blob_checksums(self, blob_file):
        ...
        content = BLOB_CONTENTS[blob_file.name]
    ...
    def test_blobs_static_linked_into_image(self, build_doc, zephyr_doc):
        """The final image must have a STATIC_LINK relationship to each linked blob."""
        linked = image_relationship_targets(build_doc, RelationshipType.STATIC_LINK)
    ...
    def test_generated_blob_is_input_of_module_library(self, build_doc, zephyr_doc):
        """A blob rendered into a C array must be an input of the library that embeds it.

        generate_inc_file_for_target() hands the blob to a helper target the
        module's library depends on, which lists it among its own sources; this
        is what proves that helper's sources are followed.
        """
        lib = next((f for f in build_doc.files if f.name.endswith(SUFFIX_USED_MODULE_LIB)), None)
        assert lib is not None, f"build.spdx: no file ending in '{SUFFIX_USED_MODULE_LIB}'"

        generated_from = {
            str(r.related_spdx_element_id)
            for r in get_relationships_for_element(
                build_doc, lib.spdx_id, RelationshipType.GENERATED_FROM
            )
        }
        blob = find_file_by_name(zephyr_doc, FILE_BLOB_GENERATED)
        assert blob is not None, f"zephyr.spdx: {FILE_BLOB_GENERATED} not found"
        assert any(ref.endswith(blob.spdx_id) for ref in generated_from), (
            f"build.spdx: {lib.name} is not GENERATED_FROM {FILE_BLOB_GENERATED}, "
            f"got {generated_from}"
        )

    def test_incbin_blob_is_input_of_image(self, build_doc, zephyr_doc):
        """A blob named by a compile definition must be an input of the image.

        Modules set those definitions globally, so the blob is recorded against
        the image rather than against whichever target compiled the .incbin.
        """
        generated_from = image_relationship_targets(build_doc, RelationshipType.GENERATED_FROM)
        blob = find_file_by_name(zephyr_doc, FILE_BLOB_INCBIN)
        assert blob is not None, f"zephyr.spdx: {FILE_BLOB_INCBIN} not found"
        assert any(ref.endswith(blob.spdx_id) for ref in generated_from), (
            f"build.spdx: {FILE_ZEPHYR_ELF} is not GENERATED_FROM {FILE_BLOB_INCBIN}, "
            f"got {generated_from}"
        )

    def test_generated_include_is_not_recorded_instead_of_blob(self, build_doc, zephyr_doc):
        """The rendered .inc must not stand in for the blob it was rendered from.

        Only the blob carries the module.yml provenance and the REUSE.toml
        license, so recording the generated array instead would lose both.
        """
        for doc, doc_name in ((build_doc, "build.spdx"), (zephyr_doc, "zephyr.spdx")):
            generated = [f.name for f in doc.files if f.name.endswith(FILE_BLOB_GENERATED_INC)]
            assert not generated, (
                f"{doc_name}: the generated {FILE_BLOB_GENERATED_INC} is recorded "
                f"in place of {FILE_BLOB_GENERATED}: {generated}"
            )
