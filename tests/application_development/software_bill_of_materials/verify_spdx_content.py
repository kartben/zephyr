...
# Zephyr is registered with NVD as an operating system, so advisories are matched
# against cpe:2.3:o:zephyrproject:zephyr:<version>:<update>:*:*:*:*:*:*
...
def expected_zephyr_cpe(zephyr_version):
    """Build the CPE expected for a Zephyr version as spelled in the VERSION file.

    CPE keeps a pre-release qualifier in its own 'update' field, so Zephyr's
    "4.4.0-rc3" becomes ...:zephyr:4.4.0:rc3:... while a plain "4.3.0" keeps the
...
class TestPackageProvenance:
    ...
    def test_zephyr_sources_cpe(self, zephyr_doc, zephyr_version):
        ...
        Unlike the purl, the CPE does not depend on which remote the tree was
        cloned from, so it is expected on every build, tagged release or not.
        """
        pkg = find_package_by_name(zephyr_doc, "zephyr-sources")
        assert pkg is not None, "zephyr.spdx: zephyr-sources package not found"
