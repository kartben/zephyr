...
{
	const struct emul_sensor_regmap *cfg = emul->cfg;
	...
	zassert_within(sensor_value_to_double(&result), expected, 2.0 * epsilon / scale + 0.000002,
		       ...
		       sensor_value_to_micro(&result), (unsigned int)cfg->num_channels);
}

static void check_sensor(size_t i)
{
	const struct emul_sensor_regmap *cfg = sensors[i].emul->cfg;

	for (size_t c = 0; c < cfg->num_channels; c++) {
		struct emul_sensor_channel ch = cfg->channels[c];
		double values[] = {-10.5, -0.5, 0, 0.5, 25.5};

		struct sensor_chan_spec spec = {.chan_type = ch.chan};
		q31_t lower, upper, epsilon;
		int8_t shift;

		zassert_ok(emul_sensor_backend_get_sample_range(sensors[i].emul, spec,
							      &lower, &upper, &epsilon, &shift));
		double scale = (double)BIT64(shift) / 2147483648.0;

		for (size_t v = 0; v < ARRAY_SIZE(values); v++) {
			double input = ch.chan == SENSOR_CHAN_PRESS ? 100 + values[v] :
				       CLAMP(values[v], lower * scale / 2, upper * scale / 2);

			check_sample(sensors[i].dev, sensors[i].emul, ch.chan, input);
		}
	}
}
...
{
	struct emul_sensor_regmap_data *data = sensors[5].emul->data;

	check_sensor(4);
	check_sensor(5);
	zassert_equal(data->regs[1] & 1U, 1, "One-shot must return to shutdown");
}
