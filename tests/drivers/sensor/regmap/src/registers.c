...
static bool valid[8];
static struct emul_sensor_regmap_data state;
...
static void use_model(const struct emul *source)
{
	...
	model = (struct emul){.cfg = source->cfg, .data = &state,
		.backend_api = &emul_sensor_regmap_backend_api};
	zassert_ok(emul_sensor_regmap_init(&model, NULL));
}
...
{
	return emul_sensor_regmap_i2c_api.transfer(&model, msgs, count, 0x48);
}
...
{
	struct emul_sensor_reg regs[] = {{.bytes = 5}};
	struct emul_sensor_regmap cfg = {.regs = regs, .num_regs = 1};

	USE(p3t);
	model.cfg = &cfg;
	zassert_equal(emul_sensor_regmap_init(&model, NULL), -EINVAL);
	cfg.num_regs = 0;
	zassert_equal(emul_sensor_regmap_init(&model, NULL), -EINVAL);
	cfg.num_regs = 257;
	zassert_equal(emul_sensor_regmap_init(&model, NULL), -EINVAL);
