...
int fce_load_rom(char *rom)
{
    ...
    memcpy(&fce_rom_header, rom + off, sizeof(fce_rom_header));
    off += sizeof(fce_rom_header);
    ...
    byte *prg_data = (byte *)(rom + off);
    off += prg_size;

    if (mmc_id == 0 || mmc_id == 3) {
        // if there is only one PRG block, we must repeat it twice
        if (fce_rom_header.prg_block_count == 1) {
            mmc_copy(0x8000, prg_data, 0x4000);
            mmc_copy(0xC000, prg_data, 0x4000);
        }
        else {
            mmc_copy(0x8000, prg_data, 0x8000);
        }
    }
    else {
    ...
    for (i = 0; i < fce_rom_header.chr_block_count; i++) {
        byte *chr_data = (byte *)(rom + off);
        off += 0x2000;
        mmc_append_chr_rom_page(chr_data);

        if (i == 0) {
            ppu_copy(0x0000, chr_data, 0x2000);
        }
    }
