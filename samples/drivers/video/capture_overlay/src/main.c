...
int main(void)
{
	...
	while (1) {
		...
		if (fmt.pixelformat == VIDEO_PIX_FMT_RGB565) {
			uint16_t *pixel = (uint16_t *)vbuf->buffer;
			uint32_t rect_x = fmt.width / 4;
			uint32_t rect_y = fmt.height / 4;
			uint32_t rect_w = fmt.width / 2;
			uint32_t rect_h = fmt.height / 2;
			...
			for (uint32_t y = 0; y < fmt.height; y++) {
				for (uint32_t x = 0; x < fmt.width; x++) {
					if (x >= rect_x && x < (rect_x + rect_w) &&
						...
						// For RGB565, buffer is typically laid out linearly row by row
						// pitch might be different from width * bytes_per_pixel
						// For this example, assume pitch is width * 2 for RGB565
						pixel[y * (fmt.pitch / 2) + x] = red_color;
					}
				}
			}
		}
