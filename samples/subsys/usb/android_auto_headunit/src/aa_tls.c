 ...
 * so the bundled self-signed one gets no further than the handshake. Supply a
 * head unit credential with the Kconfig options.
 */
...
int aa_tls_init(void)
{
	 ...
	 * certificate authority. The sample accepts any certificate, which keeps
	 * the root store out of it and lets a session be read from a capture.
	 */
