...
LOG_MODULE_REGISTER(aa_tls, CONFIG_SAMPLE_AA_HU_LOG_LEVEL);

/*
 * PEM credentials, NUL terminated as required by the Mbed TLS PEM parser. The
 * head unit presents a certificate that a phone verifies against Google's CA;
 * the bundled self-signed one is accepted only by non-verifying phones such as
 * the companion accessory sample. Supply a real one with the Kconfig options.
 */
static const unsigned char hu_cert_pem[] = {
#include "hu_cert.pem.inc"
	0x00
};
static const unsigned char hu_key_pem[] = {
#include "hu_key.pem.inc"
	0x00
};
...
int aa_tls_init(void)
{
	...
	ret = mbedtls_x509_crt_parse(&crt, hu_cert_pem, sizeof(hu_cert_pem));
	if (ret != 0) {
		log_mbedtls_error("Certificate parsing failed", ret);
		return -EIO;
	}

	ret = mbedtls_pk_parse_key(&pk, hu_key_pem, sizeof(hu_key_pem), NULL, 0);
	if (ret != 0) {
	...
	/*
	 * A production head unit verifies the phone certificate against Google's
	 * certificate authority. The sample accepts any certificate so that it
	 * works against the companion accessory sample and captures.
	 */
	...
	mbedtls_debug_set_threshold(CONFIG_SAMPLE_AA_HU_TLS_DEBUG_LEVEL);
#endif
...
/* Drive the handshake with whatever server data is currently staged; lock held */
static int handshake_pump(void)
{
	int ret;
