 ...
 * the phone is sent. Speech sits far below full scale at a digital microphone,
 * so the samples are amplified on the way; taking its sixteen-bit output
 * instead would clip everything above a fifth of the range.
 */
#define MIC_CAPTURE_WIDTH CONFIG_SAMPLE_AA_HU_MIC_CAPTURE_WIDTH

/* Bits to shift right on the way to the phone. Negative amplifies instead. */
#define MIC_GAIN_SHIFT (MIC_CAPTURE_WIDTH - MIC_WIDTH - \
...
BUILD_ASSERT(MIC_CAPTURE_WIDTH == 16 || MIC_CAPTURE_WIDTH == 24,
	     "microphone capture width must be 16 or 24 bits");
 ...
 * fills.
 */
/* A slab's blocks and its buffer both have to be word aligned */
static char mic_slab_buffer[MIC_BLOCK_COUNT * MIC_BLOCK_SIZE]
	Z_GENERIC_SECTION(CONFIG_SAMPLE_AA_HU_MIC_BUFFERS_SECTION) __aligned(sizeof(void *));
...
static void mic_convert(const uint8_t *in, uint8_t *out, size_t count)
{
	...
	for (i = 0; i < count; i++) {
		const uint8_t *p = &in[i * (MIC_CAPTURE_WIDTH / 8U)];
#if MIC_CAPTURE_WIDTH == 24
		int32_t raw = (int32_t)sys_get_le24(p);
#else
		int32_t raw = (int32_t)sys_get_le16(p);
#endif
		int32_t scaled;

		/* The accessors read unsigned; put the sign back */
...
#else
		/* Multiply rather than shift: the sample can be negative */
		scaled = raw * (1 << -(MIC_GAIN_SHIFT));
#endif
