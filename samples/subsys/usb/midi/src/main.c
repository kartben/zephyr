...
/* Structure to hold MIDI event data */
struct midi_event {
	struct midi_ump ump;
};

/* Message queue for MIDI events - queue up to 32 events */
K_MSGQ_DEFINE(midi_event_queue, sizeof(struct midi_event), 32, 4);
...
static void on_midi_packet(const struct device *dev, const struct midi_ump ump)
{
	...
	/* Only process MIDI1 channel voice messages */
	if (UMP_MT(ump) != UMP_MT_MIDI1_CHANNEL_VOICE) {
		return;
	}

	/* Queue the MIDI event for processing in the main thread */
	struct midi_event event = {.ump = ump};

	/* Try to put the event in the queue, drop it if queue is full */
	if (k_msgq_put(&midi_event_queue, &event, K_NO_WAIT) != 0) {
		LOG_WRN("MIDI event queue full, dropping event");
	}
}
...
int main(void)
{
	...
	while (1) {
		...
		while (k_msgq_get(&midi_event_queue, &event, K_NO_WAIT) == 0) {
			if (UMP_MIDI_COMMAND(event.ump) == UMP_MIDI_NOTE_ON) {
				uint8_t note = UMP_MIDI1_P1(event.ump);
				if (white_key_widgets[note]) {
					lv_obj_set_style_bg_color(white_key_widgets[note],
								  lv_color_hex(0xAAAAAA),
								  LV_PART_MAIN);
				} else if (black_key_widgets[note]) {
					lv_obj_set_style_bg_color(black_key_widgets[note],
								  lv_color_hex(0xAAAAAA),
								  LV_PART_MAIN);
				}
			} else if (UMP_MIDI_COMMAND(event.ump) == UMP_MIDI_NOTE_OFF) {
				uint8_t note = UMP_MIDI1_P1(event.ump);
				if (white_key_widgets[note]) {
					lv_obj_set_style_bg_color(white_key_widgets[note],
								  lv_color_hex(0xFFFFFF),
