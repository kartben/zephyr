...
/**
 * @file
 * @brief OCPP demo application for connecting to CitrineOS Central System
 *
 * This demo shows how to connect a Zephyr-based charge point to CitrineOS,
 * an open-source OCPP Central System implementation.
 */
...
LOG_MODULE_REGISTER(citrineos_demo, LOG_LEVEL_INF);
...
int main(void)
{
	...
	/* CitrineOS uses a different endpoint structure than SteVe */
	struct ocpp_cp_info cpi = { "CitrineOSDemo", "Zephyr", .num_of_con = NO_OF_CONN };
	struct ocpp_cs_info csi = { NULL,
				    "/ocpp",  /* CitrineOS standard OCPP endpoint */
				    CONFIG_NET_SAMPLE_OCPP_PORT,
				    AF_INET };

	printk("OCPP CitrineOS Demo %s\n", CONFIG_BOARD);
	printk("Connecting to CitrineOS Central System\n");
	...
	for (i = 0; i < NO_OF_CONN; i++) {
		snprintf(idtag[i], sizeof(idtag[0]), "CitrineID%02d", i);
