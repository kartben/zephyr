...
``custom_properties`` (an API still unreleased upstream that ``scripts/reuse_custom_properties.py``
monkey patches onto the installed ``reuse`` in the meantime). They are all Zephyr-specific: SPDX
has no non-deprecated *file*-level field
...
# Importing the reuse_custom_properties shim backports the custom_properties API used below onto
# the released reuse library (no-op once reuse provides it natively).
sys.path.append(str(ZEPHYR_BASE / "scripts"))
import reuse_custom_properties  # noqa: E402, F401
