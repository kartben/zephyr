...
This Sphinx plugin resolves C domain cross-references (``:c:func:``, ``:c:struct:``,
``:c:macro:``, ...) into links to the HTML documentation generated by Doxygen, using the
Doxygen XML output as the lookup index. It also provides a minimal ``doxygengroup``
directive that renders a link to the page of a Doxygen group.

Note that this plugin only creates links: unlike ``breathe``, it does not render API
documentation into the Sphinx pages themselves.

Configuration options
=====================

- ``doxybridge_projects``: Dictionary of Doxygen projects, mapping project name to the
  Doxygen output directory. References are looked up in every project, in insertion
...
Renders a link to the page Doxygen generated for a group::

    .. doxygengroup:: uart_interface

Options:

- ``project``: Restrict the lookup to the named ``doxybridge_projects`` entry instead of
  searching all of them. Upstream Zephyr documents a single Doxygen project, so this is
  never needed here; it exists for downstream users who bridge several projects and need
  to disambiguate a group name defined in more than one of them.
"""
...
class DoxygenReferencer(SphinxPostTransform):
    ...
    def run(self, **kwargs: Any) -> None:
        projects = self.app.config.doxybridge_projects
        if not projects:
            return
        ...
        for node in self.document.traverse(addnodes.pending_xref):
            ...
            # A doxygengroup directive may pin its reference to a single project;
            # everything else searches all of them and takes the first match.
            project = node.get("project")
            if project and project not in projects:
                logger.warning(
                    f"doxybridge: unknown Doxygen project '{project}'",
                    location=node,
                    type="doxybridge",
                )
                continue
            search = [project] if project else projects
            ...
            doxygen_target = str(projects[found_name]) + "/html/" + doxygen_target
