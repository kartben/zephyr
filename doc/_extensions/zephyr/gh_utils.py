...
def git_tracked_files() -> frozenset[str]:
    ...
    A single ``git ls-files`` call replaces the per-page
    ``git ls-files --error-unmatch`` invocations, which add up to a
    significant amount of time over the thousands of pages of a full
    documentation build.
    """
    try:
