...
particular, Zephyr relies on CMake for its build system, Python for xxx, and also requires the
Devicetree compiler (``dtc``), to build :ref:`devicetree` files.

The current minimum required version for these main dependencies are:
...
.. note::
   ...
   or other Python projects, it is recommended to use Python virtual environments_.
...
supported architectures, providing the compilers, assemblers, linkers and necessary programs to
...
.. note::
   ...
   the :ref:`nRF5340DK <nrf5340dk_nrf5340>`, the board target must be provided as:
