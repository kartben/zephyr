...
with its :ref:`modules <modules>`. These commands use :file:`zephyrproject` as
the workspace name; you can choose another name and location. You will also
install Zephyr's Python dependencies in a `Python virtual environment`_ so they
stay separate from your system Python installation.
...
#. Install Zephyr's Python dependencies:
   ...
   workspace, so the installed packages match the Zephyr version you fetched.
...
started. Later, you can use ``-p auto`` to let west decide when a pristine build
is needed.
