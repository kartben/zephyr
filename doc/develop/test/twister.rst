...
Twister harnesses are the mechanisms used to execute tests and determine their results.
Each harness provides a different approach to test execution and result validation.

Twister supports multiple harnesses, including:

- **ztest**: The native Zephyr testing framework (recommended for most Zephyr tests)
- **console**: Pattern matching on console output
- **pytest**: Python-based testing with pytest framework
- **shell**: Shell command testing
- **robot**: Robot Framework tests in Renode simulation
- **gtest**: Google Test framework for C++ testing
- **ctest**: CMake's CTest framework integration
- **power**: Power consumption measurement and validation
- **bsim**: BabbleSim Bluetooth simulation support
- **test**: Simple pass/fail validation

For comprehensive documentation on all available harnesses, including detailed
configuration options, usage examples, and best practices, see the
:ref:`harness documentation <twister_harnesses>`.

Quick Example
=============

Here's a basic example using the console harness:
...
For more examples and detailed information about each harness, refer to the
:ref:`harness documentation <twister_harnesses>`.
