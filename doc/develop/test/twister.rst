...
A test harness is a critical component that determines how Twister monitors test
execution and evaluates whether a test passes or fails. Each harness is designed
for specific testing scenarios and frameworks.

Twister supports multiple harness types including:

- :ref:`Ztest <twister_harness_ztest>` - Zephyr's native testing framework (most common)
- :ref:`Gtest <twister_harness_gtest>` - Google Test framework integration
- :ref:`Console <twister_harness_console>` - Custom regex-based output validation
- :ref:`Pytest <twister_harness_pytest>` - Python pytest integration for advanced testing
- :ref:`Robot <twister_harness_robot>` - Robot Framework for Renode simulation
- :ref:`Power <twister_harness_power>` - Power consumption measurement and validation
- :ref:`Shell <twister_harness_shell>` - Shell command testing
- :ref:`Bsim <twister_harness_bsim>` - BabbleSim simulation integration
- :ref:`Ctest <twister_harness_ctest>` - CMake CTest integration

For complete documentation on each harness including configuration options, examples,
and best practices, see the :doc:`Test Harnesses Guide <twister/harness>`.
