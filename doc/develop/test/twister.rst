...
A harness is a feature implemented by Twister that defines how test execution results are
evaluated and reported. The harness is specified in the test's configuration file using the
``harness`` keyword and determines how Twister interprets test output to determine pass/fail
status.

Twister provides multiple harnesses for different testing scenarios:

- **ztest** - For tests using Zephyr's Ztest framework
- **console** - For matching regex patterns in console output
- **pytest** - For Python-based test orchestration and validation
- **gtest** - For GoogleTest C++ tests
- **ctest** - For CMake CTest integration
- **robot** - For Robot Framework test suites (Renode simulation)
- **power** - For power consumption measurement and validation
- **display_capture** - For visual display testing with camera
- **bsim** - For BabbleSim Bluetooth simulator integration
- **shell** - For interactive shell command testing

For comprehensive documentation on each harness, including configuration options, examples,
and best practices, see :ref:`twister_harnesses_index`.

Quick Example
=============

Here's a simple example using the console harness:
...
- :ref:`twister_harnesses_index` - Comprehensive harness documentation with detailed guides
  for each harness type, including configuration options, usage examples, and best practices
...
Twister provides comprehensive support for running tests on real hardware devices, including
single device testing, multiple device testing with hardware maps, fixture support, and
quarantine features.

For complete information on hardware testing, including setup, configuration, and advanced
features, see :ref:`twister_hardware_testing`.
...
- :ref:`twister_hardware_testing` - Complete guide to hardware testing including hardware
  maps, fixtures, quarantine, and troubleshooting

Test Configuration
******************

Test configuration files allow you to customize platform selection, define test levels,
and control various aspects of test execution. These configuration files are particularly
useful for adapting Twister to different testing environments.

For complete information on test configuration files, including platform configuration,
test level definitions, and best practices, see :ref:`twister_test_configuration`.
...
- :ref:`twister_test_configuration` - Complete guide to test configuration files
...
Zephyr supports Robot Framework for automated testing with human-readable test cases.
Robot Framework tests can express interactive test scenarios in plain text format and
execute them in Renode simulation.

For comprehensive information on writing and running Robot Framework tests, see
:ref:`twister_robot_tests`.
...
- :ref:`twister_robot_tests` - Complete guide to Robot Framework testing
