...
CPE reports every CVE ever recorded against that project, including ones already fixed in the
revision Zephyr pins and ones that never applied to how Zephyr uses the code. The
   ...
   security:
     ...
     vulnerability-assessments:
       ...
       - vulnerability: CVE-2024-54321
         ...
         impact-statement: >
           The affected parser is only reachable from the server role, which Zephyr's
           integration never enables.
...
.. list-table::
   ...
   * - Status
     - Meaning
   * - ``not_affected``
     - The vulnerability does not apply to this module as Zephyr ships it.
   * - ``affected``
     - The module is affected and users should act.
   * - ``fixed``
     - The revision Zephyr pins contains the fix.
   * - ``under_investigation``
