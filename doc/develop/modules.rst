...
These references are recorded in the ``modules-deps`` document generated by :ref:`west spdx
<west-spdx>`, where vulnerability scanners pick them up — for example the ``sbom-cve-check`` tool
run by :ref:`west spdx --cve-check <west-spdx-cve-check>`.

VEX statements
--------------

The ``security`` section can also carry VEX (Vulnerability Exploitability eXchange) statements in
its ``vex`` field, asserting the module's status with respect to specific vulnerabilities that its
external references match. This is how a module records that a CVE reported against the upstream
project does not apply to the Zephyr module repository — most commonly because the fix was
cherry-picked into the module before an upstream release containing it was integrated:
...
.. code-block:: yaml
   ...
   security:
     external-references:
       ...
       - pkg:github/Mbed-TLS/mbedtls@V3.5.2
     vex:
       - vulnerability: CVE-2024-23170
         status: fixed
         notes: fix cherry-picked from upstream in zephyrproject-rtos/mbedtls@abc1234
       - vulnerability: CVE-2024-28755
         status: not_affected
         justification: vulnerable_code_not_present
         impact-statement: the affected feature is not part of the module codebase

Each statement supports the following fields:

- ``vulnerability`` (required): the vulnerability identifier, typically a CVE ID.
- ``status`` (required): one of ``fixed``, ``not_affected``, ``affected`` or
  ``under_investigation``.
- ``justification``: for ``not_affected`` statements, one of the machine-readable `VEX
  justification labels <https://github.com/openvex/spec/blob/main/OPENVEX-SPEC.md#status-justifications>`_
  (e.g. ``vulnerable_code_not_present``). A ``not_affected`` statement should carry a
  ``justification`` or an ``impact-statement`` (or both).
- ``impact-statement``: for ``not_affected`` statements, a free-form explanation of why the
  vulnerability has no impact.
- ``action-statement``: for ``affected`` statements, a free-form description of the recommended
  remediation or mitigation.
- ``notes``: free-form notes, e.g. a pointer to the cherry-picked fix.

When generating SPDX 3.0 documents, ``west spdx`` emits these statements using the SPDX Security
profile, and ``west spdx --cve-check`` (as well as any VEX-aware SBOM consumer) uses them to filter
vulnerability findings: vulnerabilities recorded as ``fixed`` or ``not_affected`` are no longer
reported as unpatched. See :ref:`west-spdx-security-profile` for details.

Statements only apply while their vulnerability matches one of the module's external references, so
entries for vulnerabilities fixed by a version bump of the external reference can be dropped
whenever the module is synchronized with a new upstream release.
