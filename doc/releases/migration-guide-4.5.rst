...
* The hardening tool's data file :file:`scripts/kconfig/hardened.csv` has been replaced by the
  YAML database :file:`scripts/kconfig/hardening.yaml`. Downstream forks that patched the CSV
  should migrate their entries to the new format (see :ref:`hardening`); out-of-tree
