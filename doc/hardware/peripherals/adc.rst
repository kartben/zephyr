...
**Channel Configuration**
  Applications can select the gain, reference voltage, acquisition time, and differential mode
  for each channel, subject to the capabilities of the underlying ADC hardware.

**Conversion Sequence**
  Conversions are requested using so called sequences that describe which channels to sample, the
  resolution oversampling settings, and the buffer to store the results.
...
* **Reference:** internal or external reference voltage used to convert the raw value to a physical
  voltage (:c:member:`adc_channel_cfg.reference`).
* **Gain:** hardware gain applied before conversion, used when computing the effective input voltage
  (:c:member:`adc_channel_cfg.gain`).
* **Acquisition time:** minimum time the sampler must track the input signal before the conversion
  starts; drivers may round this to the closest supported value
...
The exact set of supported parameters depends on the driver. Unsupported combinations will cause
:c:func:`adc_channel_setup`/:c:func:`adc_channel_setup_dt` to fail with an error code.
...
* :c:member:`adc_sequence.channels`: A bitmask of channel indices to sample.
  For example, to sample channels 0, 3, and 5, the bitmask would be ``BIT(0) | BIT(3) | BIT(5)``
  (which is ``0x29``).
* :c:member:`adc_sequence.buffer`: A pointer to a buffer to store the samples.
  The buffer must be large enough to hold all the samples from the sequence. The required size in
  bytes is calculated as:
  ...
  * ``number_of_channels`` is the number of bits set in ``channels``.
  * ``number_of_samples`` is ``1 + extra_samplings``.
  * ``sizeof(sample_type)`` is typically 2 bytes (``int16_t`` or ``uint16_t``).

* :c:member:`adc_sequence.resolution`: Resolution of the samples in bits.
* :c:member:`adc_sequence.oversampling`: Oversampling setting (log2 of the number of samples to
  average).
...
.. note::
   ...
   kernel system timer, which may have limited precision (e.g., 10-50 kHz ticks). For high-frequency
   precise sampling, hardware-specific timers or DMA modes should be considered.
...
multi-channel sequences, this means that the first sample corresponds to the lowest-numbered channel
in the bitmask, the second to the next channel, and so on.
