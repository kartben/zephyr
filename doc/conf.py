...
#
# The ``html-quick`` profile sets these tags to skip expensive autogenerated
# layers (Doxygen, Kconfig, board catalog and external board/sample/snippet
# content) when previewing narrative changes. The options below turn the
# matching generators off; the zephyr.partial_build extension degrades the
# content they would have produced (placeholder directives, plain-text
# references) so the build still passes under -W. Such a build is preview-only:
# it can neither validate cross-references nor be published.
SKIP_DOXYGEN, SKIP_KCONFIG, SKIP_BOARD_CATALOG, SKIP_EXTERNAL_CONTENT = (
    tags.has(t)  # pylint: disable=undefined-variable  # noqa: F821
    for t in ("skip_doxygen", "skip_kconfig", "skip_board_catalog", "skip_external_content")
)
