 ...
 *
 * @param domid Domain whose physmap is updated. Unprivileged callers must use ``DOMID_SELF``.
 * @param idx Index within the Xen mapping space identified by @p space.
 * @param space Mapping space selector, typically one of the ``XENMAPSPACE_*`` constants.
 ...
 *
 * @param domid Domain whose physmap is updated. Unprivileged callers must use ``DOMID_SELF``.
 * @param foreign_domid Foreign domain identifier used with ``XENMAPSPACE_gmfn_foreign``.
 * Pass ``0`` for the other mapping spaces.
 * @param space Mapping space selector, typically one of the ``XENMAPSPACE_*`` constants.
 ...
 *
 * @retval 0 If the hypercall completed successfully.
 * @retval -errno Negative error code returned by the hypercall.
 */
 ...
 *
 * @param domid Domain whose physmap is populated. Unprivileged callers must use ``DOMID_SELF``.
 * @param extent_order Extent order used by Xen, where each extent covers ``2^extent_order`` pages.
 * @param nr_extents Number of entries in @p extent_start.
 * @param mem_flags Xen memory flags. Arm guests currently pass ``0``.
 * @param extent_start Array of guest frame numbers that receive the populated extents.
 *
 * @return Number of populated extents on success.
 * @retval -errno Negative error code returned by the hypercall.
 */
 ...
 * @param frame Starting frame within the Xen resource.
 ...
 * @param[out] frame_list Buffer that receives the frame list returned by Xen.
 *
 * @retval 0 If the resource was acquired successfully.
 * @retval -errno Negative error code returned by the hypercall.
 */
