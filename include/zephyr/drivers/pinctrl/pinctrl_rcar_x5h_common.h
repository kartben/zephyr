...
#define RCAR_PIN_FLAGS_PULL_SET BIT(0) /**< Pull-up, pull-down, or bias disable is requested */
#define RCAR_PIN_FLAGS_PUEN     BIT(1) /**< Perform on/off control of the pull resistors */
#define RCAR_PIN_FLAGS_PUD      BIT(2) /**< Select pull-up resistor if set, pull-down otherwise */
#define RCAR_PIN_FLAGS_FUNC_SET BIT(3) /**< Alternate function for the pin is requested */
...
typedef struct pinctrl_soc_pin {
	...
	uint16_t drive_strength_microamp; /**< Drive strength (if output) */
} pinctrl_soc_pin_t;

/**
 * @brief Utility macro to get the alternate function ID.
 *
...
struct pfc_drive_reg {
	...
	const uint16_t pins[32]; /**< Pin IDs */
};
