 ...
 *       allocating channel in this way. See @ref z_nrf_grtc_timer_interval_set.
...
/** @brief Configure periodic interval compare event.
 *
 * @note This function configures a periodic compare event for the given channel.
 *       The event will be generated every @p interval_value ticks until
 *	 @ref z_nrf_grtc_timer_interval_stop is called.
 *
 * @param chan Channel ID.
 *
 * @param initial_value Relative compare value (in GRTC ticks) after which
 *                      the first interval event is generated.
 *
 * @param interval_value Relative interval value in GRTC ticks.
 *
 * @param handler User callback executed in the GRTC interrupt context.
 *
 * @param user_data Data passed to the handler.
 *
 * @retval 0 If successful.
 * @retval -EPERM If the given channel does not support extended features.
 */
