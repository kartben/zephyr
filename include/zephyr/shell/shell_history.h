...
/**
 * @file
 * @brief Shell command history management (internal).
 */
...
/**
 * @brief Shell history instance structure.
 */
struct shell_history {
	struct k_heap *heap;    /**< Heap for history storage. */
	sys_dlist_t list;       /**< List of history entries. */
	sys_dnode_t *current;   /**< Current history entry pointer. */
};
 ...
 * @param[in]     up      Direction (true for up, false for down).
 * @param[out]    dst     Buffer where line is copied.
 * @param[in,out] len     Buffer size (input), amount of copied data (output).
 *
 * @retval true  Remains in history mode.
 * @retval false Exited history mode.
 */
 ...
 * @param history Shell history instance.
 *
 * @retval true  In browsing mode.
 * @retval false Not in browsing mode.
 */
