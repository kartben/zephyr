...
/**
 * @brief MQTT TX buffer structure.
 */
...
/**
 * @brief MQTT-based shell transport structure.
 */
struct shell_mqtt {
	...
	uint8_t rx_rb_buf[RX_RB_SIZE];          /**< RX ring buffer data. */
	...
	struct mqtt_client mqtt_cli;            /**< The MQTT client struct. */
	struct buffer {
		uint8_t rx[SH_MQTT_BUFFER_SIZE]; /**< RX buffer. */
		uint8_t tx[SH_MQTT_BUFFER_SIZE]; /**< TX buffer. */
	} buf;                                  /**< Buffers for MQTT client. */
	struct k_mutex lock;                    /**< Lock mutex. */
	...
	struct mqtt_publish_param pub_data;     /**< Publish data. */
	...
	struct k_work_delayable connect_dwork;  /**< Connect delayed work. */
	struct k_work_delayable subscribe_dwork; /**< Subscribe delayed work. */
	struct k_work_delayable process_dwork;  /**< Process delayed work. */
	struct k_work_delayable publish_dwork;  /**< Publish delayed work. */
	enum sh_mqtt_transport_state {
		SHELL_MQTT_TRANSPORT_DISCONNECTED, /**< Transport disconnected. */
		SHELL_MQTT_TRANSPORT_CONNECTED,    /**< Transport connected. */
	} transport_state;                      /**< MQTT connection states. */
	enum sh_mqtt_subscribe_state {
		SHELL_MQTT_NOT_SUBSCRIBED, /**< Not subscribed. */
		SHELL_MQTT_SUBSCRIBED,     /**< Subscribed. */
	} subscribe_state;                      /**< MQTT subscription states. */
...
/**
 * @brief Get pointer to shell MQTT backend instance.
 *
...
/**
 * @brief Get the device ID for the shell MQTT backend.
 *
 * Function to define the device ID (devid) for which the shell MQTT backend uses as a
