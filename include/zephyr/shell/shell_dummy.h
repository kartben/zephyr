...
/**
 * @brief Shell dummy transport structure.
 */
struct shell_dummy {
	bool initialized;     /**< Initialized flag. */
	size_t len;           /**< Current number of bytes in buffer (0 if no output). */
	char buf[CONFIG_SHELL_BACKEND_DUMMY_BUF_SIZE]; /**< Output buffer to collect shell output. */
};
...
/**
 * @brief Get the buffered output in the shell and reset the pointer.
 *
...
/**
 * @brief Clear the output buffer in the shell backend.
 *
 * @param sh	Shell pointer.
 */
